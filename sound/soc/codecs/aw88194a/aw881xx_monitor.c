	...
	struct aw881xx *aw88194, uint16_t *reg_val)
{
	...
	if ((aw88194 == NULL) || (reg_val == NULL)) {
		aw_dev_err(aw881xx->dev, "%s: aw88194 is %p, reg_val=%p\n",
					...
					uint16_t gain_value)
{
	...
	if (aw881xx == NULL) {
		aw_dev_err(aw881xx->dev, "%s: aw881xx is %p\n",
			__func__, aw881xx);
		return ret;
	}
...
/*
static int aw881xx_monitor_get_bst_ipeak(
	struct aw881xx *aw881xx, uint16_t *reg_val)
{
	int ret = -1;

	if ((aw881xx == NULL) || (reg_val == NULL)) {
		aw_dev_err(aw881xx->dev, "%s: aw881xx is %p, reg_val=%p\n",
			__func__, aw881xx, reg_val);
		return ret;
	}

	ret = aw881xx_reg_read(aw881xx, AW881XX_REG_SYSCTRL2, reg_val);
	*reg_val = *reg_val & (AW881XX_BIT_SYSCTRL2_BST_IPEAK_MASK);
					...
					uint16_t *voltage)
{
	...
	if ((aw881xx == NULL) || (voltage == NULL)) {
		aw_dev_err(aw881xx->dev, "%s aw881xx is %p, voltage=%p\n",
			__func__, aw881xx, voltage);
		return ret;
	}
					...
					int16_t *temp)
{
	...
	if ((aw881xx == NULL) || (temp == NULL)) {
		aw_dev_err(aw881xx->dev, "%s aw881xx= %p, temp=%p\n",
			__func__, aw881xx, temp);
		return ret;
	}
					...
					uint16_t *gain_db)
{
	...
	if ((aw881xx == NULL) || (bst_ipeak == NULL) || (gain_db == NULL)) {
		aw_dev_err(aw881xx->dev,
			"aw881xx is %p, bst_ipeak is %p, gain_db is %p\n",
			aw881xx, bst_ipeak, gain_db);
		return ret;
	}
			...
			uint16_t *bst_ipeak, uint16_t *gain_db, uint16_t *vmax)
{
	...
	if ((aw881xx == NULL) || (bst_ipeak == NULL) ||
		(gain_db == NULL) || (vmax == NULL)) {
		aw_dev_err(aw881xx->dev,
			"%s: aw881xx=%p, bst_ipeak=%p, gain_db=%p, vmax=%p\n",
			__func__, aw881xx, bst_ipeak, gain_db, vmax);
		return ret;
	}
...
static int aw881xx_monitor_check_sysint(struct aw881xx *aw881xx)
{
	...
	if (aw881xx == NULL) {
		aw_dev_err(aw881xx->dev, "%s: aw881xx is NULL\n", __func__);
		return ret;
	}
...
/*
void aw881xx_monitor_cal_ipeak(uint16_t *real_ipeak,
	uint16_t vol_ipeak, uint16_t temp_ipeak)
{
	if (real_ipeak == NULL) {
		aw_dev_err(aw881xx->dev, "%s: real_ipeak=%p\n",
			__func__, real_ipeak);
		return;
	}

	if (vol_ipeak == AW881XX_IPEAK_NONE &&
		temp_ipeak == AW881XX_IPEAK_NONE)
		*real_ipeak = AW881XX_IPEAK_NONE;
	else
		*real_ipeak = (vol_ipeak < temp_ipeak ? vol_ipeak : temp_ipeak);
}

void aw881xx_monitor_cal_gain(uint16_t *real_gain,
	uint16_t vol_gain, uint16_t temp_gain)
{
	if (real_gain == NULL) {
		aw_dev_err(aw881xx->dev, "%s: real_gain=%p\n",
			__func__, real_gain);
		return;
	}
...
*/
void aw881xx_monitor_work(struct aw881xx *aw881xx)
{
	int ret;
	...
	ret =
	    aw881xx_monitor_check_temperature(aw881xx,
...
int aw881xx_monitor_stop(struct aw881xx_monitor *monitor)
{
...
int aw881xx_monitor_start(struct aw881xx_monitor *monitor)
{
