...
#define AW881XX_VERSION "v0.1.12"
...
//prize add  by lipengpeng 20201014 start 
static const struct snd_soc_dapm_widget aw881xx_dapm_widgets[]={
/* playback */
SND_SOC_DAPM_AIF_IN("AIF_RX_l","Speaker_Playback_l",0,SND_SOC_NOPM,0,0),
SND_SOC_DAPM_AIF_IN("AIF_RX_r)","Speaker_Playback_r",0,SND_SOC_NOPM,0,0),
SND_SOC_DAPM_OUTPUT("audio_out_l"),
SND_SOC_DAPM_OUTPUT("audio_out_r"),
};

static const struct snd_soc_dapm_route aw881xx_audio_map[]={
{"audio_out_l",NULL,"AIF_RX_l"},
{"audio_out_r",NULL,"AIF_RX_r"},
};
...
void aw881xx_append_suffix(char *format, const char **change_name,
			struct aw881xx *aw881xx)
{
	...
	snprintf(buf, 50, format, *change_name, aw881xx->name_suffix);
...
int aw881xx_reg_reads(struct aw881xx *aw881xx,
	uint8_t reg_addr, uint8_t *buf, uint16_t len)
{
	int ret = -1;

	mutex_lock(&aw881xx->i2c_lock);
	ret = aw881xx_i2c_reads(aw881xx, reg_addr, buf, len);
	if (ret < 0)
...
void aw881xx_run_pwd(struct aw881xx *aw881xx, bool pwd)
{
	aw_dev_dbg(aw881xx->dev, "%s: enter\n", __func__);
...
int aw881xx_get_sysint(struct aw881xx *aw881xx, uint16_t *sysint)
{
	...
	if (ret < 0)
		aw_dev_info(aw881xx->dev, "%s: read sysint fail, ret=%d\n",
				__func__, ret);
	else
		*sysint = reg_val;
...
static int aw881xx_dsp_set_vcalb(struct aw881xx *aw881xx)
{
	...
	aw_dev_dbg(aw881xx->dev,
			"%s: icalk=%d, vcalk=%d, vcalb=%d, reg_val=%d\n",
...
static int aw881xx_update_dsp_cfg(struct aw881xx *aw881xx)
{
	...
	aw881xx_dsp_container_update(aw881xx, aw881xx->dsp_cfg_cnt,
		AW881XX_SPK_DSP_CFG_ADDR);
...
static void aw881xx_stop(struct aw881xx *aw881xx)
{
	...
	aw881xx_run_mute(aw881xx, true);
	if (aw881xx->dsp_cfg_cnt) {
		aw881xx_update_dsp_cfg(aw881xx);
	} else {
		aw_dev_info(aw881xx->dev, "%s: dsp_cfg_cnt not ready\n", __func__);
	}
...
int aw881xx_read_product_id(struct aw881xx *aw881xx)
{
	...
	ret = aw881xx_reg_read(aw881xx, AW881XX_REG_PRODUCT_ID, &reg_val);
	if (ret < 0) {
		dev_err(aw881xx->dev, "%s: failed to read REG_EFRL: %d\n",
			__func__, ret);
		return -EIO;
	}
					...
					uint16_t base)
{
	...
	uint32_t tmp_len = 0;
	uint16_t cnt = 0;
	uint8_t dsp_data[AW881XX_MAX_RAM_WRITE_BYTE_SIZE];
#else
	uint16_t reg_val = 0;
	uint16_t dsp_val = 0;
#endif
	...
	for (retry_cnt = 0; retry_cnt < 5; retry_cnt++) {
		/* i2c writes */
		aw881xx_reg_write(aw881xx, AW881XX_REG_DSPMADD, base);
		for (i = 0; i < aw881xx_cont->len; i += AW881XX_MAX_RAM_WRITE_BYTE_SIZE) {
			...
			aw881xx_reg_writes(aw881xx, AW881XX_REG_DSPMDAT,
					&aw881xx_cont->data[i], tmp_len);
		}
		/* i2c reads */
		aw881xx_reg_write(aw881xx, AW881XX_REG_DSPMADD, base);
		for (i = 0; i < aw881xx_cont->len;
			i += AW881XX_MAX_RAM_WRITE_BYTE_SIZE) {
			if ((aw881xx_cont->len - i) < AW881XX_MAX_RAM_WRITE_BYTE_SIZE)
				tmp_len = aw881xx_cont->len - i;
			else
				tmp_len = AW881XX_MAX_RAM_WRITE_BYTE_SIZE;

			aw881xx_reg_reads(aw881xx, AW881XX_REG_DSPMDAT,
				dsp_data, tmp_len);
			for (cnt = 0; cnt < tmp_len; cnt++) {
				if (dsp_data[cnt] != aw881xx_cont->data[i + cnt]) {
					aw_dev_err(aw881xx->dev, "%s: find dsp write data err, write data[%d]=0x%02x, read data[%d]=0x%02x\n",
						__func__, i + cnt, aw881xx_cont->data[i + cnt],
						i + cnt, dsp_data[cnt]);
					dsp_match = 0;
					break;
				} else {
		...
		if (dsp_match == 1) {
			aw_dev_info(aw881xx->dev, "%s: dsp write/read match check pass\n",
				__func__);
			break;
		}
	}
	...
	for (retry_cnt = 0; retry_cnt < 5; retry_cnt++) {
		...
		for (i = 0; i < aw881xx_cont->len; i += 2) {
			reg_val = (aw881xx_cont->data[i] << 8) + aw881xx_cont->data[i + 1];
			aw881xx_reg_write(aw881xx, AW881XX_REG_DSPMDAT, reg_val);
		}
		/* i2c read */
		aw881xx_reg_write(aw881xx, AW881XX_REG_DSPMADD, base);
		for (i = 0; i < aw881xx_cont->len; i += 2) {
			dsp_val = (aw881xx_cont->data[i] << 8) +
				aw881xx_cont->data[i + 1];
			aw881xx_reg_read(aw881xx, AW881XX_REG_DSPMDAT, &reg_val);
			if (dsp_val != reg_val) {
				aw_dev_err(aw881xx->dev, "%s: find dsp write data err, write data[%d]=0x%04x, read_data[%d]=0x%04x\n",
					__func__, i, dsp_val, i, reg_val);
				dsp_match = 0;
				break;
			} else {
				dsp_match = 1;
...
static void aw881xx_dsp_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	kfree(aw881xx->dsp_cfg_cnt);
	aw881xx->dsp_cfg_cnt = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->dsp_cfg_cnt) {
		release_firmware(cont);
	...
	aw881xx->dsp_cfg_cnt->len = cont->size;
	memcpy(aw881xx->dsp_cfg_cnt->data, cont->data, cont->size);
	release_firmware(cont);
	...
	for (i = 0; i < aw881xx->dsp_cfg_cnt->len; i += 2) {
		tmp_val = aw881xx->dsp_cfg_cnt->data[i + 0];
		aw881xx->dsp_cfg_cnt->data[i + 0] = aw881xx->dsp_cfg_cnt->data[i + 1];
		aw881xx->dsp_cfg_cnt->data[i + 1] = tmp_val;
	}
	aw881xx_dsp_container_update(aw881xx, aw881xx->dsp_cfg_cnt,
...
{
	struct aw881xx *aw881xx = context;
	struct aw881xx_container *aw881xx_cfg;
	uint8_t tmp_val = 0;
	int ret = -1, i;
	...
	if (aw881xx->work_flag == true) {
		mutex_lock(&aw881xx->lock);
		for (i = 0; i < aw881xx_cfg->len; i += 2) {
			tmp_val = aw881xx_cfg->data[i + 0];
			aw881xx_cfg->data[i + 0] = aw881xx_cfg->data[i + 1];
			aw881xx_cfg->data[i + 1] = tmp_val;
		}
...
static void aw881xx_delay_work_routine(struct work_struct *work)
{
	int ret = -1;
	struct aw881xx *aw881xx =
		container_of(work, struct aw881xx, delay_work.work);
...
int aw881xx_update_hw_params(struct aw881xx *aw881xx)
{
	uint16_t reg_val = 0;
	...
	aw881xx_i2c_write_bits(aw881xx, AW881XX_REG_PLLCTRL1,
					...
					unsigned int reg)
{
	...
	if (aw881xx_reg_access[reg] & REG_RD_ACCESS) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw881xx->dev, "%s: read register failed\n",
					__func__);
			return ret;
		}
	} else {
 ...
 *
 ******************************************************/
void aw881xx_interrupt_setup(struct aw881xx *aw881xx)
{
	uint16_t reg_val = 0;
...
void aw881xx_interrupt_handle(struct aw881xx *aw881xx)
{
	uint16_t reg_val = 0;
				 ...
				 struct device_node *np)
{
	...
	if (aw881xx->reset_gpio < 0) {
		...
		dev_info(aw881xx->dev, "%s: reset gpio provided ok\n",
			__func__);
	}

	aw881xx->irq_gpio = of_get_named_gpio(np, "irq-gpio", 0);
	if (aw881xx->irq_gpio < 0)
		dev_info(aw881xx->dev, "%s: no irq gpio provided.\n", __func__);
	else
		dev_info(aw881xx->dev, "%s: irq gpio provided ok.\n", __func__);
					...
					struct device_node *np)
{
	...
	if (ret) {
		...
		dev_info(aw881xx->dev, "%s: monitor-flag = %d\n",
	...
	if (ret) {
		...
		dev_info(aw881xx->dev, "%s: monitor-timer-val = %d\n",
				...
				const char *buf, size_t count)
{
	...
	if (2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {
		...
		aw881xx->dsp_addr = (unsigned int)databuf[0];
		aw881xx_reg_write(aw881xx, AW881XX_REG_DSPMADD, databuf[0]);
	...
	if (aw881xx->dsp_cfg == AW881XX_DSP_BYPASS) {
		...
		if (ret < 0) {
			...
			aw_dev_dbg(aw881xx->dev, "%s: aw881xx_dsp_firmware:\n",
					__func__);
			aw881xx_reg_write(aw881xx, AW881XX_REG_DSPMADD,
					AW881XX_SPK_DSP_FW_ADDR);
 ...
 *
 ******************************************************/
int aw881xx_i2c_probe(struct i2c_client *i2c,
			     const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(aw881xx->reset_gpio)) {
		aw881xx_append_suffix("%s_%s", &aw881xx_rst, aw881xx);
	...
	if (gpio_is_valid(aw881xx->irq_gpio)) {
		aw881xx_append_suffix("%s_%s", &aw881xx_int, aw881xx);
	...
	aw881xx_append_suffix("%s-%s", &dai->name, aw881xx);
	aw881xx_append_suffix("%s_%s", &dai->playback.stream_name, aw881xx);
	aw881xx_append_suffix("%s_%s", &dai->capture.stream_name, aw881xx);
	...
	if (gpio_is_valid(aw881xx->irq_gpio) &&
		...
		aw881xx_append_suffix("%s_%s", &aw881xx_irq_name, aw881xx);
	...
	INIT_DELAYED_WORK(&aw881xx->delay_work, aw881xx_delay_work_routine);

	aw_dev_info(&i2c->dev, "aw88194a %s: probe completed successfully!\n", __func__);
...
int aw881xx_i2c_remove(struct i2c_client *i2c)
{
