...
static unsigned int hl7005_get_ce(void)
{
	unsigned char val = 0;
	hl7005_read_interface((unsigned char)(HL7005_CON1),
				(unsigned char *)(&val),
...
static __maybe_unused unsigned int hl7005_get_hz_mode(void)
{
	unsigned char val = 0;
	hl7005_read_interface((unsigned char)(HL7005_CON1),
				(unsigned char *)(&val),
...
static int hl7005_enable_charging(struct charger_device *chg_dev, bool en)
{
	...
	if (en) {
		if (((struct hl7005all_info *)chg_dev->dev.driver_data)->dev_model == DEVICE_ETA6937){
			...
			hl7005_reg_config_interface(0x06, 0x7A);	//safe set to 4.4v  7A---2A
...
//prize add by lipengpeng 20200114 start
static int hl7005_is_chip_enabled(struct charger_device *chg_dev, bool *en)
{
	int ret = 0;
	//ret = hl7005_get_hz_mode();
	ret = hl7005_get_ce();
...
static int hl7005_is_charging_enabled(struct charger_device *chg_dev, bool *en)
{
	unsigned int val = 0;
	val = hl7005_get_ce();
...
static int hl7005_enable_chip(struct charger_device *chg_dev, bool en)
{
	unsigned int status = 0;
	...
	if (en) {
		if (((struct hl7005all_info *)chg_dev->dev.driver_data)->dev_model == DEVICE_ETA6937){
			...
			hl7005_reg_config_interface(0x06, 0x5A);	//safe set to 4.4v
...
//prize add by lipengpeng 20200114 end
...
static int hl7005_set_mivr(struct charger_device *chg_dev, u32 uV)
{
	int ret = 0;
...
static int hl7005_get_mivr_state(struct charger_device *chg_dev, bool *in_loop)
{
	//struct hl7005all_info *chip = dev_get_drvdata(&chg_dev->dev);
...
//prize add by lipengpeng 20200114 start
	.is_enabled = hl7005_is_charging_enabled,
	.is_chip_enabled = hl7005_is_chip_enabled,
	.enable_chip = hl7005_enable_chip,
//prize add by lipengpeng 20200114 end
	...
	.set_mivr = hl7005_set_mivr,
	.get_mivr_state = hl7005_get_mivr_state,
	/*OTG*/
...
static int hl7005_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	switch(info->dev_model){
		...
		#else //elif defined(CONFIG_PSC5415A_68MR_SUPPORT)
			hl7005_reg_config_interface(0x06, 0x7A);  //old 7A
		#endif
			break;
		}
		case DEVICE_PSC5425:{
		...
		case DEVICE_ETA6937:{
			hl7005_reg_config_interface(0x06, 0x7A);//old 7A
			hl7005_reg_config_interface(0x07, 0x0C);	//vsp+0,iinlim 0x0A--800mA   0x0D 2A  0x0B--1.2A 0x0c---1.5A
			break;
		}
		default:{
	...
	switch(info->dev_model){
		case DEVICE_PSC5415A:
			hl7005_reg_config_interface(0x05, 0x43);	//some version require Bit2 must be set 1
			break;
		case DEVICE_ETA6937:
			hl7005_reg_config_interface(0x05, 0x43);	//vsp4.2+n*80mV
			hl7005_reg_config_interface(0x07, 0x0C);	//vsp+0,iinlim   0x0A--1A   0x0D 2A  0x0B--1.2A  0x0c---1.5A
			break;
		case DEVICE_PSC5425:
		default:
			hl7005_reg_config_interface(0x05, 0x43);
			break;
	}

	hl7005_reg_config_interface(0x04, 0x2A);	/* 146mA */  //2A--1.6A
