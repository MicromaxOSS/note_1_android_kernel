...
static void _set_vbus(int is_on)
{
	...
	if (is_on && !vbus_on) {
		...
		charger_dev_set_boost_current_limit(primary_charger, 1100000); /* prize modify by liaoxingen for k66601q mmx customer need 20201012 */
#else
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			if (is_host_active(mtk_musb)) {
				...
				in_uart_mode) {
			pr_info("%s USB in UART mode, to USB mode\n",
					__func__);
