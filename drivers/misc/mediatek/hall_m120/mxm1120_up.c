...
#include <linux/uaccess.h>
#include "mxm1120.h"
 //prize  add  for proc/hall1/m1120_up by zhuzhengjiang    20190112-begin
#include <linux/proc_fs.h>
 //prize  add  for proc/hall1/m1120_up by zhuzhengjiang    20190112-end
/***********************************************************/
/*customer config*/
/***********************************************************/
...
short  m1120_up_get_value(void);
static int m1120_get_calidata(void);
static int calidata_up_value = 0;
extern char *saved_command_line;
static int calidata_up_value_offset= 0;
int calidata_flag = 0;
#define DEFAULT_CALIDATA_UP_VALUE      400
static int m1120_get_calidata(void)
{
    char * ptr;
   // int calidata_up;
    ptr = strstr(saved_command_line, "hall_up_cali_data=");
    if(ptr != NULL) {
        ptr += strlen("hall_up_cali_data=");
        calidata_up_value = simple_strtol(ptr, NULL, 10);
    }
    else
        calidata_up_value = DEFAULT_CALIDATA_UP_VALUE;
    //printk("calidata up =%d \n",calidata_up_value);
    return calidata_up_value;
}
int m1120_up_state(void)
{
    int state =-1;
    short raw_data =0 ;
    raw_data = m1120_up_get_value();
    //printk("calidata_down =%d \n",calidata_down_value);
    if((calidata_up_value>0) && (raw_data > (calidata_up_value-calidata_up_value_offset)))
	state =M1120_STATE_FINISN;
    else 
	state =M1120_STATE_START;	
    printk("m1120_up_flag  calidata_up_value =%d calidata_up_value_offset=%d raw_data=%d state=%d \n",calidata_up_value,calidata_up_value_offset,raw_data,state);
    return state;
}
...
short  m1120_up_get_value(void)
{
    ...
    if (p_m1120_data != NULL)
    {
       ...
       //calidata= m1120_get_calidata();
        printk("m1120 up  raw_data (%d)  calidata_up_value=%d\n", raw,calidata_up_value);
    }
    return abs(raw);
}
/*prize  add  for hall  calibration by zhuzhengjiang    20190221-end*/
static int m1120_measure(m1120_data_t *p_data, short *raw)
{
    ...
    if (err)
    {
        printk("1120_up m1120_measure err  (%d)\n", err);
        return err;
    	}
#if 0//add By James.
    // (2) collect data
    ...
    if (m1120_get_debug(&client->dev)) {
        printk("1120_up raw data (%d)\n", *raw);
    }
    printk("1120_up raw_data (%d)\n", *raw);
    return err;
}
...
static DEVICE_ATTR(enable_up,  S_IRUGO|S_IWUSR|S_IWGRP, m1120_enable_show, m1120_enable_store);
...
/**************************************************
   ...
   **************************************************/
//prize  add  for proc/hall1/m1120_up by zhuzhengjiang    20190112-begin
static struct proc_dir_entry *m1120_up_proc_entry;

static int m1120_up_read_proc(struct seq_file *m, void *v)
{
    short raw = 0;
    //char value[2] = {0};
    raw =m1120_up_get_value();
    seq_printf(m, "%d", raw);

    //printk("m1120_up_read_proc m1120_up:%d\n", raw);
    return 0;
}

static int m1120_up_open(struct inode *inode, struct file *file)
{
    return single_open(file, m1120_up_read_proc, inode->i_private);
}

static ssize_t m1120_up_write_proc(struct file *filp, const char __user *buff, size_t count, loff_t *data)
{
	char temp[8] = {0};
	int len = 0;
	//int ret = 0;

	if (m1120_up_proc_entry == NULL) {
		printk("m1120_up_proc_entry pointer null \n");
		return -1;
	}
...
static const struct file_operations m1120_up = {
	.owner = THIS_MODULE,
	.open = m1120_up_open,
	.write = m1120_up_write_proc,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
//prize  add  for proc/hall1/m1120_calibrate_flag by zhuzhengjiang    20190415-begin
static int m1120_calibrate_flag_read_proc(struct seq_file *m, void *v)
{

    seq_printf(m, "%d", calidata_flag);

    return 0;
}

static int m1120_calibrate_flag_open(struct inode *inode, struct file *file)
{
    return single_open(file, m1120_calibrate_flag_read_proc, inode->i_private);
}

static ssize_t m1120_calibrate_flag_write_proc(struct file *filp, const char __user *buff, size_t count, loff_t *data)
{
	char temp[8] = {0};
	int len = 0;
	//int ret = 0;
	...
	if (copy_from_user(temp, buff, len))
		return 0;
	temp[len] = '\0';
	calidata_flag = temp[0] -'0';
	return count;
}
static const struct file_operations m1120_calibrate_flag = {
	.owner = THIS_MODULE,
	.open = m1120_calibrate_flag_open,
	.write = m1120_calibrate_flag_write_proc,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
//prize  add  for proc/hall1/m1120_calibrate_flag by zhuzhengjiang    20190415-end
int m1120_up_node_create(void)
{
	m1120_up_proc_entry = proc_mkdir("hall1", NULL);
	if (m1120_up_proc_entry == NULL) {
		printk("create_proc_entry  FAILED!");
		return -1;
	}

	proc_create("m1120_up", 0664, m1120_up_proc_entry, &m1120_up);

	printk("create_proc_entry  m1120_up SUCCESS.");
	proc_create("m1120_calibrate_flag", 0664, m1120_up_proc_entry, &m1120_calibrate_flag);
	return 0;
}
 //prize  add  for proc/hall1/m1120_up by zhuzhengjiang    20190112-end
static int m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk(KERN_INFO "======>lkh allocation memory for p_m1120_data up %s\n", __func__);
    ...
    //prize  add  for proc/hall1/m1120_up by zhuzhengjiang    20190112-begin
    m1120_up_node_create();
    //prize  add  for proc/hall1/m1120_up by zhuzhengjiang    20190112-end
    /*prize  add  for hall  calibration by zhuzhengjiang    20190221-begin*/
    calidata_up_value = m1120_get_calidata();
    calidata_up_value_offset = calidata_up_value/20;
    /*prize  add  for hall  calibration by zhuzhengjiang    20190221-end*/
    return 0;
...
static const struct of_device_id m1120_of_match[] = {
   // { .compatible = "magnachip,mxm1120,up_1", },
    { .compatible = "magnachip,mxm1120,up", },
   // { .compatible = "magnachip,mxm1120,up_3", },
   // { .compatible = "magnachip,mxm1120,up_4", },
    { },
};
