...
static const int sgm37895_current[][SGM37895_LEVEL_NUM] = {
	{	 60,80,  382, 480,  600,  800,  1000},
	{	 60,80,382, 480,  600,  800,  1000},
};
static const unsigned char sgm37895_flash_level[][SGM37895_LEVEL_NUM] = {
/*1500*/	{	0x69,0x55,0x3d,0x7d, 0x69, 0x55, 0x41},	
/*2000*/	{ 	0x69,0x55,0x3d,0x7d, 0x69, 0x55, 0x41},	
};
...
static int sgm37895_enable(void)
{
	...
	val = sgm37895_flash_level[chip->current_cfg][sgm37895_level];
	val = val|chip->cs;
	ret = sgm37895_write_reg(sgm37895_i2c_client, 00, val);
	if (sgm37895_is_torch(sgm37895_level)) {
...
static int sgm37895_disable(void)
{
	...
	pr_debug("%s status %x\n",__func__,sgm37895_read_reg(sgm37895_i2c_client, 0x00));
...
static int sgm37895_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_GET_DUTY_CURRENT:
		...
		pr_debug("FLASH_IOC_GET_DUTY_CURRENT(%d): %d\n",
				channel, (int)fl_arg->arg);
		fl_arg->arg = sgm37895_current[chip->current_cfg][fl_arg->arg];
		break;
		...
		struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	unsigned int cs = 0, current_cfg = 0;
	...
	if (np){
		if (of_property_read_u32(np,"cs",&cs)){
			pr_err("Can not find \"cs\" in i2c dev node.\n");
			return -EINVAL;
		}else{
			if (cs){
				chip->cs = 1;
		...
		if (of_property_read_u32(np,"current_cfg",&current_cfg)){
			pr_err("Can not find \"current_cfg\" in i2c dev node.\n");
		}else{
			if (current_cfg == 2000){
				chip->current_cfg = 1;
			}else{
