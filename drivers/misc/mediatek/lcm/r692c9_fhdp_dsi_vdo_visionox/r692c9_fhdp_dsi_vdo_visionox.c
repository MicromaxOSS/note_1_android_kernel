...
static struct LCM_setting_table lcm_initialization_setting[] = {
	...
	{0xC2,1,{0x08}},
	//{0x36,1,{0x02}},
	{0x51,2,{0x07,0xFF}},//prize-add wyq 20181226 set backlight on after logo is ready, to fix abnomal display issue when boot up   //{0x51,2,{0x0F,0xFF}}
	{0x35,1,{0x00}},
...
int is_suspend_second = 0;
int gbm_mode_second = 0;
static void lcm_suspend(void)
{
	...
	is_suspend_second = 1;
	gbm_mode_second = 0;//lcd will exit hbm mode automatically when suspend it
...
{
	LCM_LOGI("%s,nt35695 mode: mode = %d, gbm_mode_second=%d is_suspend_second=%d\n", __func__, mode, gbm_mode_second, is_suspend_second);

	//enter hbm
	if (mode == 1 && gbm_mode_second == 0) {
		gbm_mode_second = 1;

		if (is_suspend_second == 1)
	...
	//exit hbm
	else if (mode == 0 && gbm_mode_second == 1) {
		gbm_mode_second = 0;
...
struct LCM_DRIVER r692c9_fhdp_dsi_vdo_visionox_lcm_drv = {
