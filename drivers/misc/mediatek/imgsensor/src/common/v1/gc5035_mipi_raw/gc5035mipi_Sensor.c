...
#define MULTI_WRITE 1 // prize add  by zhuzhengjiang 20200728
...
// prize add  by zhuzhengjiang 20200728 start
#if MULTI_WRITE
#define I2C_BUFFER_LEN 400
#else
...
#endif
// prize add  by zhuzhengjiang 20200728 end
	...
	kal_uint8 *data, kal_uint16 length)
{
	...
	if ((((addr & 0x1fff) >> 3) + length) > GC5035_OTP_DATA_LENGTH) {
		cam_pr_debug("out of range, start addr: 0x%.4x, length = %d\n",
			addr & 0x1fff, length);
		return;
	}
...
static void gc5035_gcore_read_reg(void)
{
	...
	cam_pr_debug("register update flag = 0x%x\n", pRegs->flag);
			...
			for (j = 0; j < GC5035_OTP_REG_REG_PER_GROUP; j++)
				if (GC5035_OTP_CHECK_1BIT_FLAG(reg[base_group],
					...
					pRegs->reg[pRegs->cnt].value =
						...
						GC5035_OTP_REG_BYTE_PER_REG
						+ 2];
					cam_pr_debug("register[%d] P%d:0x%x->0x%x\n",
						pRegs->cnt,
						pRegs->reg[pRegs->cnt].page,
						pRegs->reg[pRegs->cnt].addr,
						pRegs->reg[pRegs->cnt].value);
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((check % 255 + 1) ==
				...
				cam_pr_debug("module_id = 0x%x\n",
					module_info.module_id);
				cam_pr_debug("lens_id = 0x%x\n",
					module_info.lens_id);
				cam_pr_debug("data = %d-%d-%d\n",
					module_info.year,
					module_info.month,
					module_info.day);
			} else
				cam_pr_debug("check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					info[GC5035_OTP_MODULE_DATA_SIZE - 1],
					(check % 255 + 1));
			break;
		...
		case GC5035_OTP_FLAG_INVALID2: {
			cam_pr_debug("group %d is invalid!\n", idx + 1);
			break;
		}
		default:
			break;
...
static void gc5035_otp_read_wb_info(void)
{
	...
	kal_uint16 wb_checksum_offset = GC5035_OTP_WB_CHECKSUM_OFFSET;
	kal_uint16 wb_start_offset = GC5035_OTP_WB_DATA_OFFSET;
	kal_uint16 golden_start_offset = GC5035_OTP_GOLDEN_DATA_OFFSET;
	kal_uint16 value_temp_r = 0;
	kal_uint16 value_temp_gr = 0;
	kal_uint16 value_temp_gb = 0;
	kal_uint16 value_temp_b = 0;
	kal_uint16 value_temp_g = 0;
	...
	cam_pr_debug("gc5035_otp_read_wb_info flag = 0x%x\n", flag);

	if (flag  & 0xFF) {
		switch (GC5035_OTP_GET_2BIT_FLAG(flag, 2 * (1 - idx))) {
		case GC5035_OTP_FLAG_EMPTY: {
			cam_pr_debug("wb group %d is empty!\n", idx + 1);
			pWB->flag = pWB->flag | GC5035_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			cam_pr_debug("wb group %d is valid!\n", idx + 1);
			// read unit awb
			wb_start_offset = GC5035_OTP_WB_DATA_OFFSET+(4-flag) * 0x88;
			cam_pr_debug("wb group %d is valid! start add =0x%x \n", idx + 1,wb_start_offset);
			gc5035_otp_read_group(wb_start_offset, &wb[0],
				GC5035_OTP_WB_DATA_SIZE);

			for (i = 0; i < GC5035_OTP_WB_DATA_SIZE - 1; i++){
				wb_check += wb[i];
				cam_pr_debug("unit wb wb[%d]=0x%x wb_check=%d \n", i,wb[i],wb_check);
			}

                   // read golden awb
			golden_start_offset = GC5035_OTP_WB_DATA_OFFSET+(4-flag) * 0x88+0x8*8;
			cam_pr_debug("wb group %d is valid! golden_start_offset  =0x%x \n", idx + 1,golden_start_offset);
			gc5035_otp_read_group(golden_start_offset,
				&golden[0], GC5035_OTP_GOLDEN_DATA_SIZE);
			
			for (i = 0; i < GC5035_OTP_GOLDEN_DATA_SIZE; i++){
				golden_check += golden[i];
				cam_pr_debug("golden wb golden[%d]=0x%x wb_check=%d \n", i,golden[i],golden_check);
			}

			// get check value
                   gc5035_otp_read_group(wb_checksum_offset, &wb[8],1);
			cam_pr_debug("wb_check=%d read from otp checksum :%d \n", i,golden[i],wb_check+golden_check,wb[8]);
			if (((wb_check+golden_check) % 255 + 1) == wb[8]) {

				// unit awb 
				value_temp_r = wb[0] << 8 | wb[1];
				value_temp_gr = wb[2] << 8 | wb[3];
				value_temp_gb = wb[4] << 8 | wb[5];
				value_temp_b = wb[6] << 8 | wb[7];
                         cam_pr_debug("unit wb  r/gr/gb/b (%d %d %d %d)\n",value_temp_r,value_temp_gr,value_temp_gb,value_temp_b);
				value_temp_g = (value_temp_gb+value_temp_gr +1) >> 1;

				pWB->rg = GC5035_OTP_WB_RG_TYPICAL * value_temp_r / value_temp_g;
				pWB->bg = GC5035_OTP_WB_RG_TYPICAL * value_temp_b / value_temp_g;
				pWB->rg = pWB->rg == 0 ?
					GC5035_OTP_WB_RG_TYPICAL : pWB->rg;
				pWB->bg = pWB->bg == 0 ?
					GC5035_OTP_WB_BG_TYPICAL : pWB->bg;
				pWB->flag = pWB->flag | GC5035_OTP_FLAG_VALID;
				cam_pr_debug("wb r/g = 0x%x\n", pWB->rg);
				cam_pr_debug("wb b/g = 0x%x\n", pWB->bg);

				// golden awb
				value_temp_r = golden[0] << 8 | golden[1];
				value_temp_gr = golden[2] << 8 | golden[3];
				value_temp_gb = golden[4] << 8 | golden[5];
				value_temp_b = golden[6] << 8 | golden[7];
                         cam_pr_debug("golden wb  r/gr/gb/b (%d %d %d %d)\n",value_temp_r,value_temp_gr,value_temp_gb,value_temp_b);
				value_temp_g = (value_temp_gb+value_temp_gr +1) >> 1;

				pGolden->rg = GC5035_OTP_WB_RG_TYPICAL * value_temp_r/value_temp_g;
				pGolden->bg = GC5035_OTP_WB_RG_TYPICAL * value_temp_b/value_temp_g;
				pGolden->rg = pGolden->rg == 0 ?
					GC5035_OTP_WB_RG_TYPICAL : pGolden->rg;
				pGolden->bg = pGolden->bg == 0 ?
					GC5035_OTP_WB_BG_TYPICAL : pGolden->bg;
				pGolden->flag = pWB->flag;
				cam_pr_debug("golden r/g = 0x%x\n",
						pGolden->rg);
				cam_pr_debug("golden b/g = 0x%x\n",
						pGolden->bg);

			} else {
				pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
				cam_pr_debug("wb check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
		...
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
			cam_pr_debug("wb group %d is invalid!\n", idx + 1);
			pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
			break;
		}
		default:
		...
		case GC5035_OTP_FLAG_EMPTY: {
			cam_pr_debug("golden group %d is empty!\n", idx + 1);
			pGolden->flag = pGolden->flag | GC5035_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			cam_pr_debug("golden group %d is valid!\n", idx + 1);
			golden_start_offset = GC5035_OTP_GOLDEN_DATA_OFFSET
				+ GC5035_OTP_GET_OFFSET(idx *
				GC5035_OTP_GOLDEN_DATA_SIZE);
			cam_pr_debug("wb group %d is valid! golden_start_offset  =%d \n", idx + 1,golden_start_offset);
			gc5035_otp_read_group(golden_start_offset,
				&golden[0], GC5035_OTP_GOLDEN_DATA_SIZE);
			...
			if ((golden_check % 255 + 1) ==
				...
				pGolden->rg = (golden[0] | ((golden[1] &
						0xf0) << 4));
				pGolden->bg = (((golden[1] & 0x0f) << 8) |
						golden[2]);
				pGolden->rg = pGolden->rg == 0 ?
					GC5035_OTP_WB_RG_TYPICAL : pGolden->rg;
				pGolden->bg = pGolden->bg == 0 ?
				...
				cam_pr_debug("golden check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
		...
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
			cam_pr_debug("golden group %d is invalid!\n", idx + 1);
...
static kal_uint8 gc5035_otp_read_sensor_info(void)
{
	...
	for (i = 0; i < GC5035_OTP_DATA_LENGTH; i++)
		cam_pr_debug("addr = 0x%x, data = 0x%x\n",
...
static void gc5035_otp_update_dd(void)
{
	...
	if (pDPC->flag == GC5035_OTP_FLAG_VALID) {
		cam_pr_debug("DD auto load start!\n");
...
static void gc5035_otp_update_wb(void)
{
	...
	cam_pr_debug("pWB->flag =%d pGolden->flag=%d typical rg = 0x%x, bg = 0x%x\n",pWB->flag,pGolden->flag, rg_typical, bg_typical);
	...
	if (GC5035_OTP_CHECK_1BIT_FLAG(pWB->flag, 0)) {
		...
		cam_pr_debug("channel gain r = 0x%x, g = 0x%x, b = 0x%x\n",
...
static void gc5035_otp_update_reg(void)
{
	...
	cam_pr_debug("reg count = %d\n", gc5035_otp_data.regs.cnt);

	if (GC5035_OTP_CHECK_1BIT_FLAG(gc5035_otp_data.regs.flag, 0))
		for (i = 0; i < gc5035_otp_data.regs.cnt; i++) {
			write_cmos_sensor(0xfe,
					gc5035_otp_data.regs.reg[i].page);
			write_cmos_sensor(gc5035_otp_data.regs.reg[i].addr,
					gc5035_otp_data.regs.reg[i].value);
			cam_pr_debug("reg[%d] P%d:0x%x -> 0x%x\n", i,
				gc5035_otp_data.regs.reg[i].page,
				gc5035_otp_data.regs.reg[i].addr,
				gc5035_otp_data.regs.reg[i].value);
		}
}
...
static void gc5035_otp_function(void)
{
	...
	printk("gc5035_otp_function entry");
	write_cmos_sensor(0xfa, 0x10);
	write_cmos_sensor(0xf5, 0xe9);
	write_cmos_sensor(0xfe, 0x02);
	write_cmos_sensor(0x67, 0xc0);
	...
	if (flag == 1) {
		cam_pr_debug("otp id mismatch, read again");
		memset(&gc5035_otp_data, 0, sizeof(gc5035_otp_data));
		for (i = 0; i < GC5035_OTP_ID_SIZE; i++)
...
static void set_dummy(void)
{
	...
	cam_pr_debug("Exit! framelength = %d\n", frame_length);
}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	cam_pr_debug("Exit! cal_shutter = %d, ", cal_shutter);
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	cam_pr_debug("Exit! GC5035_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
...
// prize add  by zhuzhengjiang 20200728 start
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_gc5035[] = 	{
    0xfc, 0x01,
    0xf4, 0x40,
    0xf5, 0xe9,
    0xf6, 0x14,
...
kal_uint16 addr_data_pair_preview_gc5035[] = 	{
    0xfe, 0x00,
    0x3e, 0x01,
    0xfc, 0x01,
    0xf4, 0x40,
...
kal_uint16 addr_data_pair_capture_gc5035[] = 	{
    0xf9, 0x82,
    0xfa, 0x00,
    0xfc, 0x81,
    0xfe, 0x00,
...
kal_uint16 addr_data_pair_normal_video_gc5035[] = 	{
    0xfe, 0x00,
    0x3e, 0x01,
    0xfc, 0x01,
    0xf4, 0x40,
...
kal_uint16 addr_data_pair_hs_video_gc5035[] = 	{
    0xfe, 0x00,
    0x3e, 0x01,
    0xfc, 0x01,
    0xf4, 0x40,
...
kal_uint16 addr_data_pair_slim_video_gc5035[] = 	{
    0xfe, 0x00,
    0x3e, 0x01,
    0xfc, 0x01,
    0xf4, 0x40,
...
static void sensor_init(void)
{
	cam_pr_debug("E\n");
	/* SYSTEM */
#if MULTI_WRITE // prize add  by zhuzhengjiang 20200728
	table_write_cmos_sensor(
		addr_data_pair_init_gc5035,
		sizeof(addr_data_pair_init_gc5035) / sizeof(kal_uint16));
#else
	write_cmos_sensor(0xfc, 0x01);
	write_cmos_sensor(0xf4, 0x40);
	write_cmos_sensor(0xf5, 0xe9);
...
#if MULTI_WRITE // prize add  by zhuzhengjiang 20200728
	table_write_cmos_sensor(
		addr_data_pair_preview_gc5035,
		sizeof(addr_data_pair_preview_gc5035) / sizeof(kal_uint16));
#else
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3e, 0x01);
	write_cmos_sensor(0xfc, 0x01);
...
#if MULTI_WRITE // prize add  by zhuzhengjiang 20200728
	table_write_cmos_sensor(
		addr_data_pair_capture_gc5035,
		sizeof(addr_data_pair_capture_gc5035) / sizeof(kal_uint16));
#else
	write_cmos_sensor(0xf9, 0x82);
	write_cmos_sensor(0xfa, 0x00);
	write_cmos_sensor(0xfc, 0x81);
...
#if MULTI_WRITE // prize add  by zhuzhengjiang 20200728
	table_write_cmos_sensor(
		addr_data_pair_normal_video_gc5035,
		sizeof(addr_data_pair_normal_video_gc5035) / sizeof(kal_uint16));
#else
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3e, 0x01);
	write_cmos_sensor(0xfc, 0x01);
...
#if MULTI_WRITE // prize add  by zhuzhengjiang 20200728
	table_write_cmos_sensor(
		addr_data_pair_hs_video_gc5035,
		sizeof(addr_data_pair_hs_video_gc5035) / sizeof(kal_uint16));
#else
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3e, 0x01);
	write_cmos_sensor(0xfc, 0x01);
...
#if MULTI_WRITE // prize add  by zhuzhengjiang 20200728
	table_write_cmos_sensor(
		addr_data_pair_slim_video_gc5035,
		sizeof(addr_data_pair_slim_video_gc5035) / sizeof(kal_uint16));
#else
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x3e, 0x01);
	write_cmos_sensor(0xfc, 0x01);
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	mdelay(30); //prize add  by zhuzhengjiang 20200728
	return ERROR_NONE;
}
