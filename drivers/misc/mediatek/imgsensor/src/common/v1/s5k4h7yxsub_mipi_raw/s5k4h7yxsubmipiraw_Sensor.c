...
#define PFX "s5k4h7yxsub_camera_sensor"
#define LOG_INF(format, args...)	pr_debug(PFX "[%s] " format, __func__, ##args)


static DEFINE_SPINLOCK(imgsensor_drv_lock);

extern bool update_otp_sub(void);
//extern bool check_sum_flag_lsc(void);
/*prize  liaoxingen modify  search camera 200200914 start*/
extern int curr_sensor_id;
/*prize  liaoxingen modify  search camera 200200914 end*/

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7YXSUB_SENSOR_ID,

	.checksum_value = 0xd0fe109e,//0xf16e8197,
...
static kal_uint16 read_cmos_subsensor(kal_uint32 addr)
{
...
kal_uint16 otp_4h7_read_cmos_subsensor(kal_uint32 addr)
{
	return read_cmos_subsensor(addr);
}
...
void otp_4h7sub_write_cmos_sensor_8(kal_uint16 addr, kal_uint8 para)
{
	write_cmos_sensor_8(addr, para);
}
unsigned char S5K4H7_read_cmos_subsensor(u32 addr)
{
	kal_uint16 get_byte = 0;
...
void S5K4H7_write_cmos_subsensor(u16 addr, u32 para)
{
    char pusendcmd[3] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF)};
...
static kal_uint32 return_sensor_id(void)
{
    ...
    printk("fsd----4h7 return_sensor_id\n");
	get_byte = (read_cmos_subsensor(0x0000) << 8) | read_cmos_subsensor(0x0001);
	return get_byte;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_8(0X0100, 0X00);
	/*prize  add for ATA checksum change every time by zhuzhengjiang close otp 20190307 start*/
	//fengshangdong add at 20190129 for LSC OTP 
#if 0
	write_cmos_sensor_8(0x3400, 0x00);
    write_cmos_sensor_8(0x0B00, 0x01);
#endif
	//prize end
	/*prize  add for ATA checksum change every time by zhuzhengjiang 20190307 end*/
}	/*	sensor_init  */
...
static void preview_setting(void)
{
	...
	for (i = 0; i < 100; i++)
	{
	     framecnt = read_cmos_subsensor(0x0005); /* waiting for sensor to  stop output  then  set the  setting */
	     if (framecnt == 0xFF)
	     {
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor_8(0X0100, 0X00);
	 ...
	 for (i = 0; i < 100; i++)
	{
	    framecnt = read_cmos_subsensor(0x0005);
	     if (framecnt == 0xFF)
	     {
		 LOG_INF("stream is  off\\n");
		  break;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id() + 1;
			printk("s5k4h7mipiraw open sensor_id = %x\r\n", sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
	...
	if (imgsensor_info.sensor_id != sensor_id)
		return ERROR_SENSOR_CONNECT_FAIL;
	/*prize  add for ATA checksum change every time by zhuzhengjiang 20190307 close otp start*/
#if 0
	if (!(update_otp_sub()))
	{
		LOG_INF("Demon_otp update_otp_sub error!");
	}
#endif
	/*prize  add for ATA checksum change every time by zhuzhengjiang 20190307 end*/
...
/* enable = false; */
	if (enable) {
		...
		/*prize  add for ATA checksum change every time by zhuzhengjiang  set digital gain 1x 20190307 start*/
							 ...
							 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_REGISTER:
			sensor_reg_data->RegData = read_cmos_subsensor(sensor_reg_data->RegAddr);
			break;
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
			spin_lock(&imgsensor_drv_lock);
		        /* prize modified by lifenfen,refer to commit id: 91ad11 , 20190627 begin */
			#if 0
			imgsensor.current_fps = *feature_data;
			#else
			imgsensor.current_fps = (UINT16)*feature_data_32;
			#endif
			/* prize modified by lifenfen,refer to commit id: 91ad11 , 20190627 end */
			spin_unlock(&imgsensor_drv_lock);
			break;
