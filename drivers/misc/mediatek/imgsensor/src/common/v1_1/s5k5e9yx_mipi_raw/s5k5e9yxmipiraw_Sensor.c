...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,	/* SENSOR_OUTPUT_FORMAT_RAW_Gr, */ // prize fengshangdong modify
	.mclk = 24,
...
static struct imgsensor_struct imgsensor = {
	.mirror = IMAGE_HV_MIRROR,	/* mirrorflip information */ // prize fengshangdong modify
...
//prize fengshangdong modify at 20190213
/* Sensor output window information */
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
 { 2592, 1944,	 0,  	0, 2592, 1944, 1296,  972, 0,	0, 1296,  972, 0, 0, 1296,  972}, // Preview
 { 2592, 1944,	 16,  	12, 2560, 1920, 2560, 1920, 0,	0, 2560, 1920, 0, 0, 2560, 1920}, // capture
 { 2592, 1944,	 16,  	12, 2560, 1920, 2560, 1920, 0,	0, 2560, 1920, 0, 0, 2560, 1920}, // video
...
static void write_cmos_sensor(kal_uint16 addr, kal_uint16 para)
{
	...
	iWriteRegI2CTiming(
	pusendcmd, 4, imgsensor.i2c_write_id, imgsensor_info.i2c_speed);
}
...
static void write_cmos_sensor_8(kal_uint16 addr, kal_uint8 para)
{
	...
	iWriteRegI2CTiming(
	pusendcmd, 3, imgsensor.i2c_write_id, imgsensor_info.i2c_speed);
}
...
#endif
#if  0
static kal_uint16 s5k5e9yx_table_write_cmos_sensor
...
static void hdr_write_shutter(kal_uint16 le, kal_uint16 se)
{
	...
	le = (le < imgsensor_info.min_shutter)
		? imgsensor_info.min_shutter : le;
 ...
 *
 * PARAMETERS
 *	bEnable: KAL_TRUE -> enable night mode,
 *	otherwise, disable night mode
 *
 * RETURNS
 *	None
 *
...
static void sensor_init(void)
{
  ...
  write_cmos_sensor_8(0x0000, 0x04);
...
static void preview_setting(void)
{
  ...
  //prize fengshangdong modify at 20190213
  write_cmos_sensor_8(0x0345, 0x08);// 0x08
  write_cmos_sensor_8(0x0346, 0x00);
  write_cmos_sensor_8(0x0347, 0x08);//0x08
  write_cmos_sensor_8(0x0348, 0x0A);
  write_cmos_sensor_8(0x0349, 0x27);//0x27
  write_cmos_sensor_8(0x034A, 0x07);
  write_cmos_sensor_8(0x034B, 0x9F);// 0x9F
  //prize end
...
static void capture_setting(kal_uint16 currefps)
{
  ...
  write_cmos_sensor_8(0x0344, 0x00);
  //prize fengshangdong modify at 20190213
  write_cmos_sensor_8(0x0345, 0x18);// 0x08
  write_cmos_sensor_8(0x0346, 0x00);
  write_cmos_sensor_8(0x0347, 0x14);//0x08
  write_cmos_sensor_8(0x0348, 0x0A);
  write_cmos_sensor_8(0x0349, 0x17);//0x27
  write_cmos_sensor_8(0x034A, 0x07);
  write_cmos_sensor_8(0x034B, 0x93);// 0x9F
  write_cmos_sensor_8(0x034C, 0x0A);
  write_cmos_sensor_8(0x034D, 0x00);//0x20
  write_cmos_sensor_8(0x034E, 0x07);
  write_cmos_sensor_8(0x034F, 0x80);// 0x98
  //prize end
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable)
		write_cmos_sensor_8(0x0100, 0x01); // prize fengshangdong modify
	else
		write_cmos_sensor_8(0x0100, 0x00); // prize fengshangdong modify
		mdelay(33);
	return ERROR_NONE;
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			LOG_INF("s5k5e9 Read sensor id fail, i2c_write_id: 0x%x\n",
				imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
...
"Exit! shutter =%d, framelength =%d/%d, dummy_line=%d, auto_extend=%d\n",
		...
		dummy_line, read_cmos_sensor(0x0342));
}
static kal_uint32 open(void)
{
	...
	/* sensor have two i2c address 0x20,0x5a
	 * we should detect the module used i2c address
	 */
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
 ...
 *
 * PARAMETERS
 * image_window : address pointer of pixel numbers
 * in one period of HSYNC
 *
...
UINT32 S5K5E9YX_MIPI_RAW_SensorInit(
