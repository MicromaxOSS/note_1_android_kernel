...
#include "gc2385mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.frame_time_delay_frame = 2,//  The delay frame of setting frame length dual camera
	.ihdr_support = 0,
	.ihdr_le_firstline = 0,
	.sensor_mode_num = 3,
	...
	.custom2_delay_frame = 3, // add for dual camera
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0, 0, 1600, 1200} // custom2  // add for dual camera
};
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 target_frame_length)  // add for dual camera
{
    ...
    spin_lock(&imgsensor_drv_lock);
      imgsensor.dummy_line = (target_frame_length > imgsensor_info.pre.framelength) ? (target_frame_length - imgsensor_info.pre.framelength) : 0;
      imgsensor.frame_length = imgsensor_info.pre.framelength + imgsensor.dummy_line;
      imgsensor.min_frame_length = imgsensor.frame_length;
    spin_unlock(&imgsensor_drv_lock);
    LOG_INF("frame_length: %d, shutter: %d, dummy_line: %d", imgsensor.frame_length, shutter, imgsensor.dummy_line);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC2385_MIRROR);
	...
	write_cmos_sensor(0x4e, GC2385_BLK_Select_H);
	write_cmos_sensor(0x4f, GC2385_BLK_Select_L);
	...
	write_cmos_sensor(0x92, GC2385_STARTY);
	write_cmos_sensor(0x94, GC2385_STARTX);
...
static void custom2_setting(void) // add for dual camera
{

	write_cmos_sensor(0x07,0x01);
	write_cmos_sensor(0x08,0x52);//24fps
...
static kal_uint32 get_resolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *sensor_resolution)
{
	...
	sensor_resolution->SensorCustom2Width = imgsensor_info.custom2.grabwindow_width; // add for dual camera
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
		...
		break;
	case MSDK_SCENARIO_ID_CUSTOM2: // add for dual camera
