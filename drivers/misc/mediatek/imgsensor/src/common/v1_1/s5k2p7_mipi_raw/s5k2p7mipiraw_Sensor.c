...
#ifdef AGOLD_STEREOCAM_SUPPORT_XAPI_S5K2P7
...
static void custom1_setting(void)
{
	...
	/* full size 24fps*/
	table_write_cmos_sensor(addr_custom1,
				sizeof(addr_custom1) / sizeof(kal_uint16));
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	#ifdef AGOLD_S5K2P7_PDAF_VC
	sensor_info->PDAF_Support = 2;  //Glenn added to support PDAF for 2P7,20190410
	#else
...
static int xapi_thread_loop(void *arg) {
	//kal_uint8 ret1;
	kal_uint16 start_addr = 0x0D90;
	MUINT16 data_size = 2050+1;
	...
	u8 tempbuf[10];
	int32_t temp_size = 0;
	u8 oufei_id[3] = {0x07, 0x02, 0x07 };//module id:0x07 lens id:0x02 vcm id :0x07
	...
	if (1) {
		...
		if (0 == sys_ret) {
			if (sensor_power_on) {
				...
				iReadRegI2C(pu_send_cmd, 2, pdatabuff, data_size, EEPROM_I2C_ADDR);
				...
				iReadRegI2C(pu_send_cmd, 2, tempbuf, temp_size, EEPROM_I2C_ADDR);

				temp_size = 2;
				start_addr = 0x0007;
				pu_send_cmd[0] = (char)(start_addr >> 8);
				pu_send_cmd[1] = (char)(start_addr & 0xFF);
				iReadRegI2C(pu_send_cmd, 2, tempbuf + 1, temp_size, EEPROM_I2C_ADDR);
				LOG_INF("module=0x%x lensid=0x%x vcmid=0x%x \n", tempbuf[0], tempbuf[1],  tempbuf[2]);
				if (tempbuf[0] == oufei_id[0])
					pdatabuff[2050] = 1;
				else
					pdatabuff[2050] = 0;

				xapi_3d_cal_init(pdatabuff, data_size);
			}
		up(&thread_sem);
		}
		kfree(pdatabuff);
