...
#include "gc2385macromipiraw_Sensor.h"
...
// #define LOG_1 printk("liaojie gc2385GC2385, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

// #define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC2385macroDuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2385MACRO_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	printk(" lipengpeng gc2385macro curr_sensor_id=%d\n",curr_sensor_id);
		...
		do {
			...
			*sensor_id=*sensor_id+1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("liaojie gc2385macro write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			printk("liaojie gc2385macro sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
//prize add by lipengpeng 20200309 end 
	...
	spin_unlock(&imgsensor_drv_lock);
	GC2385macroDuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
...
UINT32 GC2385MACRO_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
