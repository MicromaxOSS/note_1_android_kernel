...
#include "ov02b10_mipi_Sensor.h"

/*********************Modify Following Strings for Debug********************/
#define PFX "OV02B10_camera_sensor"
#define LOG_1 cam_pr_debug("OV02B10,MIPI 1LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B10MIPI_SENSOR_ID,

	.checksum_value = 0xdac1f07c,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x12, read_cmos_sensor(0x12) | 0xff);  //set mirror & flip on
	write_cmos_sensor(0xfd, 0x03);
...
//prize add by lipengpeng 20200309 end 
		...
		while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				...
				sensor_id = return_sensor_id();
				if (sensor_id == imgsensor_info.sensor_id) {
					cam_pr_debug("ov02b10 i2c write id: 0x%x, sensor id: 0x%x\n",
						imgsensor.i2c_write_id, sensor_id);
					break;
				}
				cam_pr_debug("ov02b10 Read sensor id fail, write id: 0x%x, id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				retry--;
			} while (retry > 0);
			i++;
...
/*prize modified by zhaopengge-20200909--end*/
