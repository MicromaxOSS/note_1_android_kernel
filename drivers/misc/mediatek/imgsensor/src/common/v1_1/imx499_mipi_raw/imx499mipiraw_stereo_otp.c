...
int write_dualcam_cali_flag_file_imx499(void)
{
...
int write_dualcam_cali_result_flag_file_imx499(int result)
{
...
#if 0
int dump_dualcam_cali_flag_file_imx499(void)
{
    struct file *eeprom_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int dump_dualcam_cali_result_flag_file_imx499(int result)
{
    struct file *result_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int store_dualcam_cali_data_imx499(void)
{
    int size = -1, rc = -1;
    BYTE checkSum = 0;
	int ret = 1;
    ...
    rc = write_dualcam_cali_flag_file_imx499();
    if (rc < 0) {
    ...
    rc = write_dualcam_cali_result_flag_file_imx499(ret);
    if (rc < 0) {
...
int dump_dualcam_cali_data_imx499(void)
{
    struct file *eeprom_file = NULL;
...
#if 0
    err = dump_dualcam_cali_flag_file_imx499();
    ...
    err = dump_dualcam_cali_result_flag_file_imx499(ret2);
    if (err < 0) {
        printk("%s: failed to write_dualcam_cali_flag_file\n", __func__);
        //return -1;
        ret2 = -1;
