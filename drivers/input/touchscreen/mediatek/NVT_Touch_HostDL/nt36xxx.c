...
info_retry:
	...
	nvt_read_pid();
	#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.chip,"NVT_FW:0x%02x", ts->fw_ver);
    sprintf(current_tp_info.id,"0x%d",ts->nvt_pid);
    strcpy(current_tp_info.vendor,"NVT36672");
    sprintf(current_tp_info.more,"%d*%d",2400,1080);
	#endif
...
//prize-Automatic switching mode corresponding to frame rate and TP report rate-pengzhipeng-20201119-start

void nvt_ts_report_rate(unsigned int new_dynfps)
{
	uint8_t buf[4] = {0};
	mutex_lock(&ts->lock);
	printk("pzp nvt_ts_report_rate =%d\n", new_dynfps);
	if (new_dynfps == 9000)
	{
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = 0x76;
		CTP_SPI_WRITE(ts->client, buf, 2);
		printk("pzp nvt_ts_report_rate 9000 ok\n");
	}
	else
	{
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = 0x77;
		CTP_SPI_WRITE(ts->client, buf, 2);
		printk("pzp nvt_ts_report_rate 6000 ok\n");
	}
	mutex_unlock(&ts->lock);
}
//prize-Automatic switching mode corresponding to frame rate and TP report rate-pengzhipeng-20201119-end
...
//prize-Solve the problem of TP ESD failure-pengzhipeng-20201118-start
void nvt_ts_esd_resume(void)
{
	mutex_lock(&ts->lock);
...
//prize-Solve the problem of TP ESD failure-pengzhipeng-20201118-end
