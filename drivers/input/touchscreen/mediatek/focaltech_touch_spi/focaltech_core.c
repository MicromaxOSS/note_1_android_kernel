...
#include "mt_spi.h"
#endif
/* prize modify by liaoxingen for bug99406 20201015 start */
#include <mt-plat/mtk_boot_common.h>
/* prize modify by liaoxingen for bug99406 20201015 end */
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    if (((0xEF == buf[2]) && (0xEF == buf[3]) && (0xEF == buf[4]))
        ...
        FTS_DEBUG("touch data(%x) abnormal,ret:%d", buf[1], ret);
        return -EIO;
    }
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if((get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT)
        ...
        FTS_ERROR("not run fts_fwupg_init");
    else
...
#ifdef CONFIG_PRIZE_HARDWARE_INFO
	//fts_read_reg(FTS_REG_CHIP_ID, &ctp_fw_version);
	strcpy(current_tp_info.chip, "FT8756");
	//sprintf(current_tp_info.id, "Focaltech_FW:0x%02x", ctp_fw_version);
