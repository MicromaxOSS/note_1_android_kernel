...
/* prize modified by lifenfen, compile error: address of array 'gsx_gesture->gesture_data' will always evaluate to 'true' [-Werror,-Wpointer-bool-conversion], , 20191021 begin */
#if 0
	if (!buf || !gsx_gesture->gesture_data) {
#else
        if (!buf) {
#endif
/* prize modified by lifenfen, compile error: address of array 'gsx_gesture->gesture_data' will always evaluate to 'true' [-Werror,-Wpointer-bool-conversion], , 20191021 end */
		ts_info("Parameter error!");
		return -EPERM;
	}
	read_lock(&gsx_gesture->rwlock);
