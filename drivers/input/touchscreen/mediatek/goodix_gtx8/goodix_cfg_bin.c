...
int goodix_get_reg_and_cfg(struct goodix_ts_device *ts_dev, struct goodix_cfg_bin *cfg_bin)
{
	...
	if (!normal_pkg) {
		...
		/* prize modified by lifenfen, add esd write register, refer to GT9886 Programming Guide, 20200107 begin */
		if (IC_TYPE_NORMANDY == ts_dev->ic_type)
			ts_dev->reg.esd = 0x30F3;//TS_REG_ESD_TICK_W
		else
		/* prize modified by lifenfen, add esd write register, refer to GT9886 Programming Guide, 20200107 end */
