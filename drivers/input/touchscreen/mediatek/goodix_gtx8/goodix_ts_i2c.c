	...
	struct goodix_ts_board_data *board_data)
{
	...
	r = of_get_named_gpio(node, "goodix,ldo-gpio", 0);
	if (r < 0) {
		ts_err("Invalid ldo-gpio in dt: %d", r);
		return -EINVAL;
	} else {
		ts_info("Parse ldo-gpio[%d] from dt", r);
		board_data->ldo_gpio = r;
	}
...
/* begin, prize-lifenfen-20180104, add for touchpanel hardware info */
	goodix_i2c_read(ts_dev, 0x6F78, buf, 1);
	ts_info("config version: %02x  current_tp_length = %d", buf[0], current_tp_length);
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
        snprintf(current_tp_info.chip + current_tp_length, sizeof(current_tp_info.chip) - current_tp_length,"  cfg: %02x", buf[0]);
#endif
/* end, prize-lifenfen-20180104, add for touchpanel hardware info */
	return r;
}
...
//unsigned short force[] = {0,0x5d,I2C_CLIENT_END,I2C_CLIENT_END};
...
static int tpd_local_init(void)
{
	...
	if(i2c_add_driver(&goodix_i2c_driver)!=0)
	{
...
static struct tpd_driver_t googid_gt8x_driver = {
	.tpd_device_name = TS_DRIVER_NAME,
...
//static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO(TPD_DEVICE, (TPD_I2C_ADDR))};
...
{
	TPD_DEBUG("Goodix touch panel driver init\n");

	//i2c_register_board_info(TPD_I2C_BUS, &i2c_tpd, 1);
	tpd_get_dts_info();
	if(tpd_driver_add(&googid_gt8x_driver) < 0){
