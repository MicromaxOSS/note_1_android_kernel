...
/* prize added by lifenfen, add irq wakeup func if support gesture, fixbug V530A-189, 20191029 begin */
	{
#ifdef CONFIG_TOUCHSCREEN_GOODIX_GTX8_GESTURE
		enable_irq_wake(core_data->irq);
#endif
/* prize added by lifenfen, add irq wakeup func if support gesture, fixbug V530A-189, 20191029 end */
...
/* prize modified by lifenfen, for avdd regulator control, if not defined at dtsi do nothing, 20190228 begin */
#if 0
		core_data->avdd = regulator_get(dev,
				 ts_bdata->avdd_name);
#else
		core_data->avdd = regulator_get_optional(dev,
				 ts_bdata->avdd_name);
#endif
/* prize modified by lifenfen, for avdd regulator control, if not defined at dtsi do nothing, 20190228 end */
...
/* prize modified by lifenfen, for avdd regulator control, if not defined at dtsi do nothing, 20190228 end */
		...
		r = regulator_set_voltage(core_data->avdd, 2800000, 2800000);	/*set 3.3v*/
		if (r) {
			ts_err("regulator_set_voltage(%d) failed!\n", r);
			return r;
		}else {
			ts_err("regulator_set_voltage(%d) ok!\n", 2800000);
		}
	} else {
...
int goodix_ts_power_on(struct goodix_ts_core *core_data)
{
	...
	r = devm_gpio_request_one(&core_data->pdev->dev,
			...
			"ts_ldo_gpio");
	if (r < 0) {
		ts_err("Failed to request ts_ldo_gpio, r:%d", r);
		return r;
	}
...
static int goodix_ts_gpio_setup(struct goodix_ts_core *core_data)
{
	...
	ts_info("GPIO setup,reset-gpio:%d, ldo-gpio:%d,irq-gpio:%d",
		ts_bdata->reset_gpio,ts_bdata->ldo_gpio, ts_bdata->irq_gpio);
	/*
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	//r = goodix_ts_power_on(core_data);//prize wyq 20181204
	//if (r < 0)
	//	goto out;
...
/* prize added by lifenfen, for factory touchpanel test, 20190110 begin */
	if (r == 0)
		tpd_load_status = 1;

/* prize added by lifenfen, for factory touchpanel test, 20190110 end */
	return r;
}
