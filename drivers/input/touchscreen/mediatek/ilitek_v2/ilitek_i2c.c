...
static int ilitek_i2c_pll_clk_wakeup(u8 cmd)
{
	u8 wakeup = 0xA3;

	ipio_info("write dummy to wake up i2c pll clk\n");
	if (idev->write(&wakeup, sizeof(wakeup)) < 0) {
		ipio_err("i2c write wake up cmd failed\n");
		return -EIO;
	}
...
static int ilitek_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	idev->wrapper = ilitek_wrapper;//¡¤a¡ã¨¹
	idev->pll_clk_wakeup_func = ilitek_i2c_pll_clk_wakeup;
