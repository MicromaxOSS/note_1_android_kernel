...
int ilitek_tddi_init(void)
{
...
#if !TDDI_RST_BIND
	if (ilitek_tddi_reset_ctrl(idev->reset) < 0)//TP 驱动拉复位
	 ...
	 */
	if (ilitek_tddi_ic_dummy_check() < 0) { //Ilitek IC check
    ...
    ilitek_plat_tp_reset(); //power on TP no touch prize by lipengpeng 20200727
...
int ilitek_tddi_dev_init(struct ilitek_hwif_info *hwif)
{
	...
	return ilitek_tddi_interface_dev_init(hwif);//从这里跑 probe  函数
}
