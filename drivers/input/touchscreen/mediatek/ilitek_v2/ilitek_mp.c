...
{
	ilitek_dump_data(max_ptr, 32, core_mp.frame_len, 0, "Dump Benchmark Max");
	ilitek_dump_data(min_ptr, 32, core_mp.frame_len, 0, "Dump Benchmark Min");
}

void dump_node_type_buffer(s32 *node_ptr, u8 *name)
{
	ilitek_dump_data(node_ptr, 32, core_mp.frame_len, 0, "Dump NodeType");
}
...
void parser_ini_benchmark(s32 *max_ptr, s32 *min_ptr, int8_t type, char *desp, int frame_len, char *bchmrk_name)
{
	int i = 0, j = 0, index1 = 0, temp, count = 0;
	char str[512] = {0}, record = ',';
	s32 data[4];
	...
	ipio_debug("benchmark_str = %s\n", benchmark_str);

	for (i = 0; i < g_ini_items; i++) {
		if ((ipio_strcmp(ini_info[i].section_name, benchmark_str) != 0) ||
			ipio_strcmp(ini_info[i].key_name, benchmark_str) != 0)
			continue;
		record = ',';
		for (j = 0, index1 = 0; j <= ini_info[i].key_val_len; j++) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				if (scount > MAX_SECTION_NUM) {
					ipio_err("seq_item is over than its define (%d), abort\n", scount);
					ret = INI_ERR_OUT_OF_LINE;
					goto out;
				}
...
static s32 open_c_formula(int inCap1DAC , int inCap1Raw, int accuracy)
{
	...
	int inVadc_range = 36;
	int inVbk = 39;
	int inCbk_step = open_para_liltek.cbk_step;/* from mp.ini */
	int inCint = open_para_liltek.cint;/* from mp.ini */
	int inVdrv = open_para_liltek.tvch - open_para_liltek.tvcl;/* from mp.ini */
	int inGain = open_para_liltek.gain;/* from mp.ini */
	...
	if ((inCbk_step == 0) || (inCint == 0)) {
		if (id == ILI9881_CHIP) {
			inCbk_step = 41;
			inCint = 68;
		} else if ((id == ILI9882_CHIP) && (type == ILI_F)) {
			inCbk_step = 41;
			inCint = 47;
...
void allnode_open_cdc_result(int index, int *buf, int *dac, int *raw)
{
	...
	if (ipio_strcmp(desp, "open test(integration)_sp") == 0) {
		for (i = 0; i < core_mp.frame_len; i++)
			buf[i] = open_sp_formula(dac[i], raw[i], open_para_liltek.tvch, open_para_liltek.tvcl);
	} else if (ipio_strcmp(desp, "open test_c") == 0) {
		parser_get_int_data(desp, "accuracy", str, sizeof(str));
		accuracy = katoi(str);
...
static int codeToOhm(s32 Code, u16 *v_tdf, u16 *h_tdf)
{
	...
	int inTVCH = short_para_liltek.tvch;
	int inTVCL = short_para_liltek.tvcl;
	int inVariation = short_para_liltek.variation;
	int inTDF1 = 0;
	int inTDF2 = 0;
	int inCint = short_para_liltek.cint;
	int inRinternal = short_para_liltek.rinternal;
	s32 temp = 0;
...
static int short_test(int index, int frame_index)
{
	...
	short_para_liltek.tvch = katoi(str);

	parser_get_int_data("short test", "tvcl", str, sizeof(str));
	short_para_liltek.tvcl = katoi(str);

	parser_get_int_data("short test", "variation", str, sizeof(str));
	short_para_liltek.variation = katoi(str);

	parser_get_int_data("short test", "cint", str, sizeof(str));
	short_para_liltek.cint = katoi(str);

	parser_get_int_data("short test", "rinternal", str, sizeof(str));
	short_para_liltek.rinternal = katoi(str);

	/* 9881H, 7807G, 7807R not support cbk_step and cint read from mp.ini */
	if ((pid != ILI9881H) && (pid != ILI7807G) && (pid != ILI7807R)) {
		if ((short_para_liltek.cint == 0) || (short_para_liltek.rinternal == 0)) {
			ipio_err("Failed to get short parameter");
			core_mp.lost_parameter = true;
			return -1;
		}
	}

	ipio_info("TVCH = %d, TVCL = %d, Variation = %d, V_TDF1 = %d, V_TDF2 = %d, H_TDF1 = %d, H_TDF2 = %d, Cint = %d, Rinternal = %d\n"
		, short_para_liltek.tvch, short_para_liltek.tvcl, short_para_liltek.variation, tItems[index].v_tdf_1, tItems[index].v_tdf_2, tItems[index].h_tdf_1, tItems[index].h_tdf_2, short_para_liltek.cint, short_para_liltek.rinternal);
...
static int allnode_key_cdc_data(int index)
{
	...
	ret = idev->wrapper(cmd, 3, NULL, 0, OFF, OFF);
	if (ret < 0) {
	...
	if (core_mp.core_ver < CORE_VER_1420) {
		...
		cmd[0] = P5_X_GET_CDC_DATA;

		ret = idev->wrapper(cmd, 1, NULL, 0, OFF, OFF);
		if (ret < 0) {
	...
	ret = idev->wrapper(NULL, 0, ori, len, OFF, OFF);
	if (ret < 0) {
...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	if (idev->wrapper(cmd, core_mp.cdc_len, NULL, 0, OFF, OFF) < 0) {
		ipio_err("Write CDC command failed\n");
		ret = -EMP_CMD;
		goto out;
	}
	...
	/* Get original frame(cdc) data */
	if (idev->wrapper(NULL, 0, ori, len, OFF, OFF) < 0) {
		ipio_err("Read cdc data error, len = %d\n", len);
		ret = -EMP_GET_CDC;
		goto out;
	}
...
{
	int i, j, ret = 0, len = 0, rd_frame_num = 1;
	u8 cmd[15] = {0};
	u8 *ori = NULL;
	...
	if (tItems[index].bench_mark_multiple) {
		rd_frame_num = tItems[index].bench_mark_frame_num - 1;
	}

	if (frame0_buf == NULL) {
		frame0_buf = (s32 **)kzalloc(tItems[index].bench_mark_frame_num * sizeof(s32 *), GFP_KERNEL);
		if (ERR_ALLOC_MEM(frame0_buf)) {
			ipio_err("Failed to allocate frame0_buf mem (%ld)\n", PTR_ERR(frame0_buf));
			ret = -EMP_NOMEM;
			goto out;
		}
		for (i = 0 ; i < tItems[index].bench_mark_frame_num ; i++) {
			frame0_buf[i] = (s32*)kzalloc(core_mp.frame_len * sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(frame0_buf)) {
				ipio_err("Failed to allocate frame0_buf[%d] mem (%ld)\n", i, PTR_ERR(frame0_buf));
				ret = -EMP_NOMEM;
				goto out;
			}
		}
	...
	for (j = 0; j < rd_frame_num; j++) {

		atomic_set(&idev->cmd_int_check, ENABLE);
		/* Only have to write cmd at first time */
		if (j == 0) {
			if (idev->wrapper(cmd, core_mp.cdc_len, NULL, 0, OFF, OFF) < 0) {
				ipio_err("Write CDC command failed\n");
				ret = -EMP_CMD;
		...
		for (i = 0; i < core_mp.frame_len; i++) {
			...
			if ((tmp & 0x8000) == 0x8000)
				frame0_buf[j][i] = tmp - 65536;
			else
				frame0_buf[j][i] = tmp;

			// multiple case frame3 = frame1 - frame2
			if (tItems[index].bench_mark_multiple && rd_frame_num == j + 1)
				frame0_buf[j+1][i] = frame0_buf[j-1][i] - frame0_buf[j][i];
		}
	}

	if (tItems[index].bench_mark_multiple) {
		ilitek_dump_data(frame0_buf[0], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[0]/frame1");
		ilitek_dump_data(frame0_buf[1], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[1]/frame2");
		ilitek_dump_data(frame0_buf[2], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[2]/frame3");
	} else {
		ilitek_dump_data(frame0_buf[0], 32, core_mp.frame_len, core_mp.xch_len, "Mutual CDC combined[0]/frame1");
	}
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (core_mp.core_ver < CORE_VER_1420) {
		...
		if (idev->wrapper(cmd, 1, NULL, 0, OFF, OFF) < 0) {
...
static int create_mp_test_frame_buffer(int index, int frame_count)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].bench_mark_max0 == NULL) {
				tItems[index].bench_mark_max0 = (s32 **)kzalloc(tItems[index].bench_mark_frame_num * sizeof(s32 *), GFP_KERNEL);
				if (ERR_ALLOC_MEM(tItems[index].bench_mark_max0)) {
					ipio_err("Failed to allocate bench_mark_max0 mem\n");
					ipio_kfree((void **)&tItems[index].bench_mark_max0);
					return -ENOMEM;
				}

				for (i = 0; i < tItems[index].bench_mark_frame_num; i++) {
					tItems[index].bench_mark_max0[i] = (s32 *)kzalloc(core_mp.frame_len * sizeof(s32), GFP_KERNEL);
					if (ERR_ALLOC_MEM(tItems[index].bench_mark_max0[i])) {
						ipio_err("Failed to allocate bench_mark_max0[%d] mem\n", i);
						ipio_kfree((void **)&tItems[index].bench_mark_max0[i]);
						return -ENOMEM;
					}
				}
			}

			if (tItems[index].bench_mark_min0 == NULL) {
				tItems[index].bench_mark_min0 = (s32 **)kzalloc(tItems[index].bench_mark_frame_num * sizeof(s32 *), GFP_KERNEL);
				if (ERR_ALLOC_MEM(tItems[index].bench_mark_min0)) {
					ipio_err("Failed to allocate bench_mark_min0 mem\n");
					ipio_kfree((void **)&tItems[index].bench_mark_min0);
					return -ENOMEM;
				}
				for (i = 0; i < tItems[index].bench_mark_frame_num; i++) {
					tItems[index].bench_mark_min0[i] = (s32 *)kzalloc(core_mp.frame_len * sizeof(s32), GFP_KERNEL);
					if (ERR_ALLOC_MEM(tItems[index].bench_mark_min0[i])) {
						ipio_err("Failed to allocate bench_mark_max0[%d] mem\n", i);
						ipio_kfree((void **)&tItems[index].bench_mark_max0[i]);
						return -ENOMEM;
				}
...
static int  peak_to_peak_test(int index)
{
	...
	ipio_debug("index = %d, desp = %s bench_mark_frame_num = %d\n"
		, index, tItems[index].desp, tItems[index].bench_mark_frame_num);

	ret = create_mp_test_frame_buffer(index, tItems[index].bench_mark_frame_num);
	if (ret < 0) {
		ret = -EMP_NOMEM;
		goto out;
	}
	...
	if (tItems[index].spec_option == BENCHMARK) {
		for ( i = 0 ; i < tItems[index].bench_mark_frame_num ; i++ ) {
			...
			parser_ini_benchmark(tItems[index].bench_mark_max0[i], tItems[index].bench_mark_min0[i],
				tItems[index].type_option, tItems[index].desp, core_mp.frame_len, benchmark_str);

			dump_benchmark_data(tItems[index].bench_mark_max0[i], tItems[index].bench_mark_min0[i]);
		}
	}
	...
	for (i = 0; i < tItems[index].bench_mark_frame_num; i++) {
		for (j = 0; j < core_mp.frame_len; j++)
			tItems[index].buf[i * core_mp.frame_len + j] = frame0_buf[i][j];

		compare_MaxMin_result(index, &tItems[index].buf[i * core_mp.frame_len]);
		ilitek_dump_data(tItems[index].max_buf, 32, core_mp.frame_len, 0, "Dump max_buf:");
	}
out:
	return ret;
}
...
static int open_test_sp(int index)
{
	...
	if (ERR_ALLOC_MEM(open)) {
		ipio_err("Failed to allocate open mem (%ld)\n", PTR_ERR(open));
		ret = -EMP_NOMEM;
		goto out;
	}
...
static int open_test_cap(int index)
{
	...
	open_para_liltek.gain = katoi(str);
	...
	open_para_liltek.cbk_step = katoi(str);

	parser_get_int_data(tItems[index].desp, "cint", str, sizeof(str));
	open_para_liltek.cint = katoi(str);

	/* 9881H, 7807G, 7807R not support cbk_step and cint read from mp.ini */
	if ((pid != ILI9881H) && (pid != ILI7807G) && (pid != ILI7807R)) {
		if ((open_para_liltek.cbk_step == 0) || (open_para_liltek.cint == 0)) {
			ipio_err("Failed to get open parameter");
			ret = -EMP_PARA_NULL;
			core_mp.lost_parameter = true;
			goto out;
		}
	...
	ipio_info("gain = %d, tvch = %d, tvcl = %d, cbk_step = %d, cint = %d\n", open_para_liltek.gain, open_para_liltek.tvch, open_para_liltek.tvcl, open_para_liltek.cbk_step, open_para_liltek.cint );
...
static int key_test(int index)
{
	...
	ipio_debug("Item = %s, Frame Count = %d\n",
...
static int mp_test_data_sort_average(s32 *oringin_data, int index, s32 *avg_result)
{
	...
	if (ERR_ALLOC_MEM(u32data_buff)) {
		ipio_err("Failed to allocate u32data_buff FRAME buffer\n");
		ret = -ENOMEM;
		goto out;
	}
	...
	if (tItems[index].catalog == SHORT_TEST) {
		...
		for (i = 0; i < core_mp.frame_len; i++) {
			if (tmp[i] > max_ts[i] || tmp[i] < min_ts[i]) {
				ipio_debug("Fail num=%d, max=%d, val=%d, min=%d\n",i,max_ts[i],tmp[i],min_ts[i]);
				*result = MP_DATA_FAIL;
				return;
			}
		}
...
static int mp_compare_test_result(int index)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].catalog == PEAK_TO_PEAK_TEST) {
				for (i = 0; i < core_mp.frame_len; i++) {
					max_threshold[i] = tItems[index].bench_mark_max0[0][i];
					min_threshold[i] = tItems[index].bench_mark_min0[0][i];
				}
			} else {
				for (i = 0; i < core_mp.frame_len; i++) {
		...
		if (tItems[index].trimmed_mean && tItems[index].catalog != PEAK_TO_PEAK_TEST) {
			...
			if (tItems[index].bench_mark_multiple) {
				for (i = 0; i < tItems[index].bench_mark_frame_num; i++) {
					mp_compare_cdc_result(index, frame0_buf[i], tItems[index].bench_mark_max0[i], tItems[index].bench_mark_min0[i], &test_result);
				}
			} else {
...
static int mp_show_result(bool lcm_on)
{
	...
	int i = 0, x, y, j, csv_len = 0, pass_item_count = 0, line_count = 0, get_frame_cont = 1;
	s32 *max_threshold = NULL, *min_threshold = NULL;
	char *csv = NULL;
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].spec_option == BENCHMARK) {
			if (tItems[i].catalog == PEAK_TO_PEAK_TEST) {
				for (j = 0; j < core_mp.frame_len; j++) {
					max_threshold[j] = tItems[i].bench_mark_max0[0][j];
					min_threshold[j] = tItems[i].bench_mark_min0[0][j];
				}

				for (j = 0; j < tItems[i].bench_mark_frame_num; j++) {
					char bench_name[128] = {0};
					snprintf(bench_name, (CSV_FILE_SIZE - csv_len), "Max_Bench%d", (j+1));
					mp_compare_cdc_show_result(i, tItems[i].bench_mark_max0[j], csv, &csv_len, TYPE_BENCHMARK, max_threshold, min_threshold, bench_name, CSV_FILE_SIZE);
					snprintf(bench_name, (CSV_FILE_SIZE - csv_len), "Min_Bench%d", (j+1));
					mp_compare_cdc_show_result(i, tItems[i].bench_mark_min0[j], csv, &csv_len, TYPE_BENCHMARK, max_threshold, min_threshold, bench_name, CSV_FILE_SIZE);
				}
			} else {
				for (j = 0; j < core_mp.frame_len; j++) {
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			if (tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST) {
				...
				if (!tItems[i].bench_mark_multiple) {
			...
			if (tItems[i].bench_mark_multiple)
				get_frame_cont = tItems[i].bench_mark_frame_num;
	...
	if (pass_item_count == 0) {
		...
		if(core_mp.lost_parameter){
			...
			ipio_err("WARNING! OPEN or SHORT parameter not found in ini file!!\n");
		}
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	memset(&open_para_liltek, 0, sizeof(open_para_liltek));
	memset(&short_para_liltek, 0, sizeof(short_para_liltek));
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		tItems[i].bench_mark_multiple = false;
		tItems[i].bench_mark_frame_num = 1;
		...
		tItems[i].bench_mark_max0 = NULL;
		tItems[i].bench_mark_min0 = NULL;
...
static void mp_test_run(int index)
{
	...
	if (tItems[i].catalog == PEAK_TO_PEAK_TEST) {
		if (tItems[i].max_min_mode == 1 && tItems[i].spec_option == 1) {
			tItems[i].bench_mark_multiple = true;
			for (j = 1; j < 11; j++) {
				char tmp[8] = {0};
				/* format complete string from the name of section "_Benchmark_Data". */
				...
				tItems[i].bench_mark_frame_num = j;
			}
			ipio_debug("set bench_mark_frame_num = %d\n", tItems[i].bench_mark_frame_num);
		}
	}
	...
	if (core_mp.retry && tItems[i].item_result == MP_DATA_FAIL) {
		ipio_info("MP failed, doing retry %d times\n", tItems[i].retry_cnt);
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		if (tItems[i].catalog == TX_RX_DELTA) {
			...
			if (tItems[i].spec_option == BENCHMARK) {
				...
				if (tItems[i].bench_mark_max0 != NULL) {
					for (j = 0; j < tItems[i].bench_mark_frame_num; j++)
						if (tItems[i].bench_mark_max0[j] != NULL) {
							kfree(tItems[i].bench_mark_max0[j]);
							tItems[i].bench_mark_max0[j] = NULL;
						}
					kfree(tItems[i].bench_mark_max0);
					tItems[i].bench_mark_max0 = NULL;
				}
				if (tItems[i].bench_mark_min0 != NULL) {
					for (j = 0; j < tItems[i].bench_mark_frame_num; j++)
						if (tItems[i].bench_mark_min0[j] != NULL) {
							kfree(tItems[i].bench_mark_min0[j]);
							tItems[i].bench_mark_min0[j] = NULL;
						}
					kfree(tItems[i].bench_mark_min0);
					tItems[i].bench_mark_min0 = NULL;
				}
			}
	...
	for (i = 0; i < MAX_SECTION_NUM; i++) {
		for (j = 0; j < MP_TEST_ITEM; j++) {
			...
			if (ri.count > MP_TEST_ITEM) {
				ipio_err("Test item(%d) is invaild, abort\n", ri.count);
				return -EINVAL;
			}
...
{
	int ret = 0, retry = 5;
	u8 cmd = P5_X_MP_MODE_CHECK;
	u8 temp[3] = {0};

	idev->wait_int_timeout = 20;
	do {
		ret = idev->wrapper(&cmd, 1, temp, 3, ON, OFF);
		if (ret < 0) {
			ipio_err("Read MP All Pass command failed\n");
			core_mp.all_pass = false;
		} else {
			ipio_debug("0:%x 1:%x 2:%x\n", temp[0], temp[1], temp[2]);
			if (temp[0] == P5_X_MP_MODE_CHECK)
				break;
		}
	} while (--retry >= 0);
	...
	if (retry <= 0) {
		ipio_err("MP All Pass command failed, normal mode\n");
		core_mp.all_pass = false;
		return;
	}

	/* 0xFD:back door command header, 0x02:Normal mode */
	if (temp[2] == ilitek_calc_packet_checksum(temp, sizeof(temp) - 1) &&
		temp[0] == P5_X_MP_MODE_CHECK && temp[1] == 0x02) {
			core_mp.all_pass = true;
			ipio_info("MP mode check: Back door mode, MP All Pass\n");
	} else {
		/* 0xFD:back door command header, 0x01:Normal mode */
		core_mp.all_pass = false;
		ipio_info("MP mode check: Normal mode \n");
	}
}
...
int ilitek_tddi_mp_test_main(char *apk, bool lcm_on)
{
	...
	if (ERR_ALLOC_MEM(ini_info)) {
		ipio_err("Failed to malloc ini_info\n");
		ret = -EMP_NOMEM;
		goto out;
	}
	}
	...
	if (mp_sort_item(lcm_on) < 0) {
		ipio_err("Failed to sort test item\n");
		ret = -EMP_INI;
		goto out;
	}
