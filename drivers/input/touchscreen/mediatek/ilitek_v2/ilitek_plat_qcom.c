...
int ilitek_sysfs_add_device(struct device *dev) {
	int ret = 0, i;
...
int ilitek_sysfs_remove_device(struct device *dev) {
	int i;
...
#if SPRD_SYSFS_SUSPEND_RESUME
	ilitek_sysfs_add_device(idev->dev);
	if (sysfs_create_link(NULL,&idev->dev->kobj, "touchscreen") < 0) {
		ipio_err("Failed to create link!\n");
	}
#else
...
#if SPRD_SYSFS_SUSPEND_RESUME
	ilitek_sysfs_remove_device(idev->dev);
#endif
