...
static int ilitek_debug_node_buff_control(bool open)
{
	...
	ipio_info("Debug buf ctrl = %s\n", idev->dnp ? "Enable" : "Disable");

	if (open) {
		idev->dbf = 0;
		idev->odi = 0;
		ipio_kfree((void **)&idev->dbl);
		idev->dbl = kzalloc(TR_BUF_LIST_SIZE * sizeof(*idev->dbl), GFP_KERNEL);
		if (ERR_ALLOC_MEM(idev->dbl)) {
			ipio_err("Failed to allocate idev->dbl mem, %ld\n", PTR_ERR(idev->dbl));
			ret = -ENOMEM;
			goto out;
		}

		for (i = 0; i < TR_BUF_LIST_SIZE; i++) {
			idev->dbl[i].mark = false;
			ipio_kfree((void **)&idev->dbl[i].data);
			idev->dbl[i].data = kzalloc(TR_BUF_SIZE * sizeof(unsigned char), GFP_KERNEL);
			if (ERR_ALLOC_MEM(idev->dbl[i].data)) {
				ipio_err("Failed to allocate dbl[%d] mem, %ld\n", i, PTR_ERR(idev->dbl[i].data));
				ret = -ENOMEM;
				goto out;
			}
		}
...
static int debug_mode_get_data(struct file_buffer *file, u8 type, u32 frame_count)
{
	...
	if (ilitek_debug_node_buff_control(ENABLE) < 0) {
		ipio_err("Failed to allocate debug buf\n");
		ret = -ENOMEM;
		goto out;
	}
	...
	cmd[0] = 0xFA;
	cmd[1] = type;
	ret = idev->wrapper(cmd, 2, NULL, 0, ON, OFF);
	...
	while (write_index < frame_count) {
		...
		idev->dbl[idev->odi].mark = false;
		idev->odi = ((idev->odi + 1) % TR_BUF_LIST_SIZE);
	}
out:
	ilitek_debug_node_buff_control(DISABLE);
	return ret;
...
static ssize_t ilitek_proc_get_delta_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	ret = idev->wrapper(NULL, 0, data, read_length, OFF, OFF);
	if (ret < 0) {
	...
	len = snprintf(g_user_buf + len, PAGE_SIZE - len, "======== Deltadata ========\n");
	ipio_info("======== Deltadata ========\n");
...
static ssize_t ilitek_proc_debug_message_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	ipio_debug("f_count= %d, index = %d, mark = %d\n", idev->dbf, idev->odi, idev->dbl[idev->odi].mark);
	if (!wait_event_interruptible_timeout(idev->inq, idev->dbl[idev->odi].mark, msecs_to_jiffies(3000))) {
		ipio_err("WARNING ! there's no data received.\n");
	...
	if (ERR_ALLOC_MEM(tmpbuf)) {
		...
		ret = copy_to_user(buff, tmpbufback, send_data_len); /*idev->dbl[0] */
		goto out;
	}

	if (idev->dbl[idev->odi].mark) {
		if (idev->dbl[idev->odi].data[0] == P5_X_DEMO_PACKET_ID) {
			need_read_data_len = 43;
		} else if (idev->dbl[idev->odi].data[0] == P5_X_I2CUART_PACKET_ID) {
			type = idev->dbl[idev->odi].data[3] & 0x0F;

			data_count = idev->dbl[idev->odi].data[1] * idev->dbl[idev->odi].data[2];
		...
		} else if (idev->dbl[idev->odi].data[0] == P5_X_DEBUG_PACKET_ID || idev->dbl[idev->odi].data[0] == P5_X_DEBUG_LITE_PACKET_ID) {
			send_data_len = 0;	/* idev->dbl[0][1] - 2; */
			need_read_data_len = TR_BUF_SIZE - 8;
		}

		for (i = 0; i < need_read_data_len; i++) {
			send_data_len += snprintf(tmpbuf + send_data_len, sizeof(tmpbufback), "%02X", idev->dbl[idev->odi].data[i]);
...
static ssize_t ilitek_proc_get_debug_mode_data_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (ilitek_set_tp_data_len(DATA_FORMAT_DEBUG, false, NULL) < 0) {
	...
	/* change to demo mode */
	if (ilitek_set_tp_data_len(DATA_FORMAT_DEMO, false, NULL) < 0)
		ipio_err("Failed to set tp data length\n");
...
static ssize_t ilitek_node_ver_info_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (idev->protocol->core_ver_len == P5_X_CORE_VER_THREE_LENGTH)
		len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "Core version = %d.%d.%d\n",
			(idev->chip->core_ver >> 24) & 0xFF, (idev->chip->core_ver >> 16) & 0xFF,
			(idev->chip->core_ver >> 8) & 0xFF);
	else
		len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "Core version = %d.%d.%d.%d\n",
			(idev->chip->core_ver >> 24) & 0xFF, (idev->chip->core_ver >> 16) & 0xFF,
			(idev->chip->core_ver >> 8) & 0xFF, idev->chip->core_ver & 0xFF);
...
int get_tp_recore_ctrl(int data)
{
	...
	case ENABLE_RECORD:
		...
		ret = ilitek_tddi_ic_func_ctrl("tp_recore", 1, NULL, 0);
		mdelay(200);
		break;
	case DATA_RECORD:
		mdelay(50);
		ipio_info("Get data");
		ret = ilitek_tddi_ic_func_ctrl("tp_recore", 2, NULL, 0);
		if (ret < 0) {
			ipio_err("cmd fail\n");
			goto out;
		}
		...
		ret = ilitek_tddi_ic_func_ctrl("tp_recore", 3, NULL, 0);
		if (ret < 0) {
			ipio_err("cmd fail\n");
			goto out;
		}
	...
	case DISABLE_RECORD:
		...
		ret = ilitek_tddi_ic_func_ctrl("tp_recore", 0, NULL, 0);
		break;
...
int get_tp_recore_data(void)
{
	...
	if (idev->wrapper(NULL, 0, buf, sizeof(buf), OFF, OFF) < 0) {
...
void gesture_fail_reason(bool enable)
{
	...
	/* set symbol */
	if (ilitek_tddi_ic_func_ctrl("knock_en", 0x8, NULL, 0) < 0)
	...
	cmd[4] = 0xFF;
	cmd[5] = 0xFF;
	if (idev->wrapper(cmd, 6, NULL, 0, OFF, OFF) < 0)
	...
	if (idev->wrapper(cmd, 24, NULL, 0, OFF, OFF) < 0)
	...
	cmd[2] = 0x11;
	cmd[3] = 0x01;
	if (idev->wrapper(cmd, 4, NULL, 0, OFF, OFF) < 0)
		ipio_err("get gesture parameters failed");

}

int ilitek_tp_data_mode_ctrl(u8 *cmd) {
	int ret = 0;

	switch (cmd[0]) {
	case AP_MODE:
		if (idev->tp_suspend) {
			if (ilitek_set_tp_data_len(DATA_FORMAT_GESTURE_DEMO, false, NULL) < 0) {
				ipio_err("Failed to set demo len from gesture mode\n");
				ret = -ENOTTY;
			}
		} else {
			if (ilitek_tddi_switch_tp_mode(P5_X_FW_AP_MODE) < 0) {
				ipio_err("Failed to switch demo mode\n");
				ret = -ENOTTY;
			}
		}
		break;
	case TEST_MODE:
		...
		break;
	case DEBUG_MODE:
		if (!idev->tp_suspend && (idev->actual_tp_mode != P5_X_FW_AP_MODE)) {
			if (ilitek_tddi_switch_tp_mode(P5_X_FW_AP_MODE) < 0) {
				ipio_err("Failed to switch demo mode\n");
				ret = -ENOTTY;
				break;
		...
		if (idev->tp_suspend) {
			if (ilitek_set_tp_data_len(DATA_FORMAT_GESTURE_DEBUG, false, NULL) < 0) {
		...
		break;
	case DEBUG_LITE_ROI:
		if (ilitek_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_ROI, false, NULL) < 0) {
			ipio_err("Failed to set debug lite roi from gesture mode\n");
			ret = -ENOTTY;
		}
		break;
	case DEBUG_LITE_WINDOW:
		if (ilitek_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_WINDOW, false, NULL) < 0) {
			ipio_err("Failed to set debug lite window from gesture mode\n");
			ret = -ENOTTY;
		}
		break;
	case DEBUG_LITE_AREA:
		if (ilitek_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_AREA, false, &cmd[1]) < 0) {
			ipio_err("Failed to set debug lite area from gesture mode\n");
			ret = -ENOTTY;
		}
		break;
	default:
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ilitek_set_tp_data_len(DATA_FORMAT_DEMO_DEBUG_INFO, false, NULL);
	} else if (strcmp(cmd, "gesturedemoen") == 0) {
		if (data[1] == DISABLE)
			idev->gesture_demo_ctrl = DISABLE;
		else
			idev->gesture_demo_ctrl = ENABLE;
		ilitek_set_tp_data_len(DATA_FORMAT_GESTURE_SPECIAL_DEMO, false, NULL);
		...
		ilitek_tddi_ic_func_ctrl("sleep", SLEEP_IN, NULL, 0);
	} else if (strncmp(cmd, "deepsleepin", strlen(cmd)) == 0) {
		ilitek_tddi_ic_func_ctrl("sleep", DEEP_SLEEP_IN, NULL, 0);
		...
		ilitek_set_tp_data_len(DATA_FORMAT_DEBUG, false, NULL);
	} else if (strncmp(cmd, "switchdemomode", strlen(cmd)) == 0) {
		ilitek_set_tp_data_len(DATA_FORMAT_DEMO, false, NULL);
		...
		idev->wrapper(temp, w_len, NULL, 0, OFF, OFF);
	} else if (strncmp(cmd, "ior", strlen(cmd)) == 0) {
		int r_len = 0;
		r_len = data[1];
		ipio_info("r_len = %d\n", r_len);
		idev->wrapper(NULL, 0, temp, r_len, OFF, OFF);
		for (i = 0; i < r_len; i++)
			ipio_info("read[%d] = %x\n", i, temp[i]);
		...
		idev->wrapper(temp, w_len, NULL, 0, ON, OFF);
		...
		ilitek_tddi_ic_func_ctrl("edge_palm", data[1], NULL, 0);
		...
		ptr[1] = data[2];
		ptr[2] = data[3];
		ilitek_dump_data(ptr, 8, 3, 0, "Gesture symbol");
		...
		temp[3] = data[4];
		temp[4] = data[5];
		ret = ilitek_tp_data_mode_ctrl(temp);
	} else if (strncmp(cmd, "switchmpretry", strlen(cmd)) == 0) {
		idev->mp_retry = !idev->mp_retry;
		ipio_info("switchmpretry = %d\n", idev->mp_retry);
	} else if (strncmp(cmd, "spiice", strlen(cmd)) == 0) {
		idev->spi_mode = SPI_ICE_MODE;
	} else if (strncmp(cmd, "spislave", strlen(cmd)) == 0) {
		idev->spi_mode = SPI_SLAVE_MODE;
	} else {
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	bool slave_int_check, int_check;
	bool esd_en = idev->wq_esd_ctrl, bat_en = idev->wq_bat_ctrl;
	...
	case ILITEK_IOCTL_I2C_WRITE_DATA:
		...
		ret = idev->wrapper(szBuf, i2c_rw_length, NULL, 0, OFF, OFF);
		if (ret < 0)
	...
	case ILITEK_IOCTL_I2C_READ_DATA:
		...
		ret = idev->wrapper(NULL, 0, szBuf, i2c_rw_length, OFF, OFF);
		if (ret < 0) {
	...
	case ILITEK_IOCTL_TP_FUNC_MODE:
		...
		ret = idev->wrapper(szBuf, 3, NULL, 0, OFF, OFF);
		break;
	...
	case ILITEK_IOCTL_TP_CORE_VER:
		...
		ipio_debug("Core version = %d.%d.%d.%d, length = %d\n",
			szBuf[0], szBuf[1], szBuf[2], szBuf[3], idev->protocol->core_ver_len);
	...
	case ILITEK_IOCTL_TP_MODE_CTRL:
		...
		ret = ilitek_tp_data_mode_ctrl(szBuf);
		break;
	...
	case ILITEK_IOCTL_ICE_MODE_SWITCH:
		...
		if (ilitek_ice_mode_ctrl(szBuf[0] ? ENABLE : DISABLE,
	...
	case ILITEK_IOCTL_WRAPPER_RW:
		...
		if (wrap_int == 1) {
			...
			int_check= OFF;
			slave_int_check = (wrap_rlen > 0 ? ON : OFF);
		}

		ipio_debug("i2c_irq = %d, spi_irq = %d\n", int_check, slave_int_check);

		idev->wrapper(szBuf + 5, wrap_wlen, wrap_rbuf, wrap_rlen, slave_int_check, int_check);

		ilitek_dump_data(szBuf + 5, 8, wrap_wlen, 16, "wrap_wbuf:");
		ilitek_dump_data(wrap_rbuf, 8, wrap_rlen, 16, "wrap_rbuf:");
