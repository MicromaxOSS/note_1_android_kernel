...
static ssize_t cts_touch_glove_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
	u8  value;
	cts_fw_reg_readb(cts_dev_glove,CTS_DEVICE_FW_REG_GLOVE_MODE,&value);
    count = sprintf(buf, "Glove Mode: %s, 0x8095 = %x\n", cts_is_glove_enabled(cts_dev_glove)? "On" : "Off",value);
...
static ssize_t cts_touch_glove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	u8  value;
    if (s){
		if (!cts_is_glove_enabled(cts_dev_glove)) {
            printk("[Mode]enter glove mode");
            ret = cts_enter_glove_mode(cts_dev_glove);
            if (ret >= 0) {
				cts_fw_reg_readb(cts_dev_glove,CTS_DEVICE_FW_REG_GLOVE_MODE,&value);
				sprintf(current_tp_info.more,"Glove On 0x8095 = %x",value);
            }
        }
	}else{
		if (cts_is_glove_enabled(cts_dev_glove)) {
				printk("[Mode]exit glove mode");
				ret = cts_exit_glove_mode(cts_dev_glove);
				if (ret >= 0) {
				cts_fw_reg_readb(cts_dev_glove,CTS_DEVICE_FW_REG_GLOVE_MODE,&value);
				sprintf(current_tp_info.more,"Glove Off 0x8095 = %x",value);
            }
        }
    }
    printk("[Mode]glove mode status:  %d", cts_is_glove_enabled(cts_dev_glove));
    return count;
}
...
int cts_sysfs_add_device(struct device *dev)
{
    ...
    int ret = 0, i;
		...
		int err = 0;
		struct i2c_client *cts_i2c_client;
		cts_i2c_client = cts_data->i2c_client;
		cts_dev_glove = cts_dev;
		if (!sysfs_rootdir) {
		...
		err = sysfs_create_link(prize_glove,&cts_i2c_client->dev.kobj,"common_node");
		if (err){
			printk("prize fts sysfs_create_link fail\n");
		}
		if(sysfs_create_file(&cts_i2c_client->dev.kobj, &dev_attr_state.attr))
		{
			return -1;
		}
	#endif
