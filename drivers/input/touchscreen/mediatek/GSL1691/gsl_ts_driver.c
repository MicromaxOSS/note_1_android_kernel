        ...
        u8 reg, u8 *buf, u32 num)
{
	...
	if((NULL!=client) && (num>0) && (num<=255))
	{
		client->addr = (client->addr & I2C_MASK_FLAG) | I2C_DMA_FLAG;
	if(temp[0] < 0x80)
	{
		temp[0] = (temp[0]+8)&0x5c;
		memcpy(gsl_dma_buff_va, temp, 1);

		i2c_master_send(client,(unsigned char *)gsl_dma_buff_pa,1);	
		err = i2c_master_recv(client,(unsigned char *)gsl_dma_buff_pa,4);
	...
	{	
		...
		i2c_master_send(client,(unsigned char *)gsl_dma_buff_pa,1);
		if((i+8)<num)
			err = i2c_master_recv(client,((unsigned char *)gsl_dma_buff_pa+i),8);
		else
			err = i2c_master_recv(client,((unsigned char *)gsl_dma_buff_pa+i),(num-i));
		i+=8;
	}
	}

	memcpy(buf, gsl_dma_buff_va, num);
...
s32 gsl_write_interface(struct i2c_client *client, u8 reg, u8 *buf, u32 num)
{
	...
	else if( 1 &&(NULL != gsl_dma_buff_va))
	{
		for (i = 0; i < writelen; i++)
		{
			gsl_dma_buff_va[i] = wr_buf[i];
			//print_info("[DMA][gsl_write_interface] gsl_dma_buff_va[%d]=%x\n",i,gsl_dma_buff_va[i]);
		}

	    client->addr = (client->addr & I2C_MASK_FLAG )| I2C_DMA_FLAG;
	    ret = i2c_master_send(client, (unsigned char *)gsl_dma_buff_pa, writelen);
	    client->addr = client->addr & I2C_MASK_FLAG & ~I2C_DMA_FLAG;
...
#endif
	//add tuwenjie 20160517
	for (source_line = 0; source_line < source_len; source_line++) 
	{
		if(1 == SMBUS_TRANS_LEN)
		{
			addr =(u8) GSL_DOWNLOAD_DATA[source_line].offset;
			memcpy(&buf[0], &GSL_DOWNLOAD_DATA[source_line].val, 4);
			gsl_write_interface(client, addr, buf, 4);
		...
		else
		{
			...
			{
				buf[0] = (u8)(GSL_DOWNLOAD_DATA[source_line].val & 0x000000ff);
				gsl_write_interface(client, GSL_PAGE_REG,&buf[0] ,1 ); 	
				send_flag = 1;
			}
			else 
			{
				if (1 == send_flag % (SMBUS_TRANS_LEN < 0x08 ? SMBUS_TRANS_LEN : 0x08))
					addr = GSL_DOWNLOAD_DATA[source_line].offset;

				memcpy(&buf[cur], &GSL_DOWNLOAD_DATA[source_line].val, 4);
				cur += 4;

				if (0 == send_flag % (SMBUS_TRANS_LEN < 0x08 ? SMBUS_TRANS_LEN : 0x08)) 
				{
					gsl_write_interface(client, addr, buf ,SMBUS_TRANS_LEN*4); 	
					cur = 0;
				}
...
{
	return tpd_proximity_detect*100;
}
...
static void gsl_report_point(struct gsl_touch_info *ti)
{
	...
	if (ti->finger_num > 0){
		for (tmp = 0; ti->finger_num > tmp; tmp++) 
		{
			input_mt_slot(tpd->dev, ti->id[tmp]);
			...
			cur_touches |= BIT(ti->id[tmp]);
		}
	}

	//printk("HH last_touches %02x, cur_touches %02x\n",last_touches,cur_touches);
	//finger num change
	if (last_touches ^ cur_touches){
		for(tmp = 0; tmp < MAX_CONTACTS; tmp++){
			if ((last_touches & BIT(tmp))^(cur_touches & BIT(tmp))){//diff
				if (last_touches & BIT(tmp)){//1->0
					input_mt_slot(tpd->dev, tmp);
...
/*#ifdef GSL_TIMER
	...
	if (tpd_proximity_flag == 1)
	{
		...
		//get raw data
		print_info(" ps_buf[0] =%d, buf[1] =%d, buf[2] =%d,buf[3] =%d\n",buf[0],buf[1],buf[2],buf[3]);
		//map and store data to hwm_sensor_data
		sensor_data.values[0] = tpd_get_ps_value();
		sensor_data.value_divide = 1;
		sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
		
		print_info(" edl ps tpd_proximity_detect=%d\n",tpd_proximity_detect);		
...
*/
	...
	{
		temp_a = tmp_buf[(i + 1) * 4 + 3] & 0x0f;
		temp_b = tmp_buf[(i + 1) * 4 + 2];
		cinfo.x[i] = temp_a << 8 |temp_b;
		temp_a = tmp_buf[(i + 1) * 4 + 1];
		temp_b = tmp_buf[(i + 1) * 4 + 0];
		cinfo.y[i] = temp_a << 8 |temp_b;		
		cinfo.id[i] = ((tmp_buf[(i + 1) * 4 + 3] & 0xf0)>>4);
...
//	struct device_node *node = NULL; //crystal
	...
	{
		client->addr = 0x40;
		printk("Crystal_shen:i2c_client_GSL->addr=%d\n",client->addr);
	}
...
//			    IRQF_TRIGGER_FALLING, "touch-eint", NULL);
	ret = request_irq(touch_irq, (irq_handler_t) tpd_eint_interrupt_handler,
			    IRQF_TRIGGER_FALLING, "mtk_tpd", NULL);
			if (ret > 0) {
				ret = -1;
...
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.chip, "GSL1691");
	//sprintf(current_tp_info.id,"0x%04x",client->addr);
	buffer[0] = 0x03;
	gsl_write_interface(client,0xf0,buffer,4);
	gsl_read_interface(client,0x0,version_id,4);
	gsl_read_interface(client,0x0,version_id,4);
	sprintf(current_tp_info.id,"0x%02x%02x%02x%02x",version_id[3], version_id[2], version_id[1], version_id[0]);
	sprintf(current_tp_info.more,"touchpanel");
	sprintf(current_tp_info.vendor,"silead");
#endif
...
static void gsl_resume(struct device *h)
{
	...
	print_info("[tpd-gsl][%s][%d] aaaaaaaaaaaaaaaaaaaaaaaaaa\n",__func__,__LINE__);
		#ifndef GSL_GESTURE//crystal 0509
			enable_irq(touch_irq); 
		#endif
	#else
	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
