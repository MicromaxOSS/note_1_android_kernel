...
int ili_tddi_init(void)
{
	...
	ILI_INFO("9882N driver version = %s\n", DRIVER_VERSION);
...
#if !TDDI_RST_BIND
	if (ili_reset_ctrl(ilits->reset) < 0)
		ILI_ERR("9882N TP Reset failed during init\n");
#endif
	...
	if (ili_ic_get_info() < 0)
		ILI_ERR("9882N Chip info is incorrect\n");
...
#endif
//prize add by lipengpeng 20200817 start
    ili_tp_reset(); 
//prize add by lipengpeng 20200817 start
	ili_ic_get_core_ver();
	...
	ILI_INFO("9882N Registre touch to input subsystem\n");
	ili_input_register();
	ili_wq_ctrl(WQ_ESD, ENABLE);
	ili_wq_ctrl(WQ_BAT, ENABLE);
	ilits->boot = true;
