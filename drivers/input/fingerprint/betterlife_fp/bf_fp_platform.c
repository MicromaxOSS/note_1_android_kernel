...
#endif
/* prize added by tangcong, Beitelai-bf3690 fingerprint info, 20200722-start */
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
#include "../../../../misc/mediatek/hardware_info/hardware_info.h"
extern struct hardware_info current_fingerprint_info;
#endif
/* prize added by tangcong, Beitelai-bf3690 fingerprint info, 20200722-end */
#include "bf_fp_platform.h"

#ifdef NEED_OPT_POWER_BY_PMU
...
#ifdef NEED_OPT_POWER_BY_PMU
static int bf_vreg_setup(struct bf_device *bf_dev, const char *name, bool enable)
{
	size_t i;
	int rc;
	struct regulator *vreg;
	struct device *dev = bf_dev->device;

	for (i = 0; i < ARRAY_SIZE(bf_dev->vreg); i++) 
	{
		const char *n = vreg_conf[i].name;
		if (!strncmp(n, name, strlen(n)))
			goto found;
	}
	BF_LOG("Regulator %s not found\n", name);
	return -EINVAL;
	...
	if (enable) 
	{
		...
		if (regulator_count_voltages(vreg) > 0) 
		{
			rc = regulator_set_voltage(vreg, vreg_conf[i].vmin,
					vreg_conf[i].vmax);
			if (rc)
				BF_LOG("Unable to set voltage on %s, %d\n", name, rc);
		}
		#if 0
		rc = regulator_set_optimum_mode(vreg, vreg_conf[i].ua_load);
		if (rc < 0)
			BF_LOG("Unable to set current on %s, %d\n", name, rc);
		#endif
		rc = regulator_enable(vreg);
		if (rc) 
		{
			BF_LOG("error enabling %s: %d\n", name, rc);
			regulator_put(vreg);
			vreg = NULL;
		}
		bf_dev->vreg[i] = vreg;
...
#endif    //BF_PINCTL
	#ifdef NEED_OPT_POWER_BY_PMU
		BF_LOG("REGULATOR_NAME : %s\n" ,REGULATOR_NAME);
		bf_vreg_setup(bf_dev, REGULATOR_NAME, enable);
	#endif
    return 0;
}
...
static int bf_main_get_gpio_info (struct bf_device *bf_dev)
{
    ...
    bf_dev->pins_spi_default = pinctrl_lookup_state (bf_dev->pinctrl_gpios, "default");
    if (!bf_dev->pins_spi_default) {
        ret = PTR_ERR (bf_dev->pins_spi_default);
        BF_LOG( "can't find fingerprint pinctrl spi_default");
        //return ret;
    }
    bf_dev->pins_reset_high = pinctrl_lookup_state (bf_dev->pinctrl_gpios, "fpc_pins_rst_high");
    if (!bf_dev->pins_reset_high) {
        ret = PTR_ERR (bf_dev->pins_reset_high);
        BF_LOG( "can't find fingerprint pinctrl fpc_pins_rst_high");
        return ret;
    }

    bf_dev->pins_reset_low = pinctrl_lookup_state (bf_dev->pinctrl_gpios, "fpc_pins_rst_low");
    if (!bf_dev->pins_reset_low) {
        ret = PTR_ERR (bf_dev->pins_reset_low);
        BF_LOG( "can't find fingerprint pinctrl fpc_pins_rst_low");
        return ret;
    }

    bf_dev->pins_fp_interrupt = pinctrl_lookup_state (bf_dev->pinctrl_gpios, "fpc_eint_as_int");
    if (!bf_dev->pins_fp_interrupt) {
        ret = PTR_ERR (bf_dev->pins_fp_interrupt);
        BF_LOG( "can't find fingerprint pinctrl fpc_eint_as_int");
        return ret;
    }
...
static long bf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case BF_IOCTL_REGISTER_READ_WRITE:
        ...
        if(read_write_cmd.len < 32) {
            ...
            spi_set_dma_en(1);
			memcpy(g_bf_dev->image_buf, read_write_cmd.data_tx, 2);
#ifdef USE_SPI1_4GB_TEST
            spi_dma_exchange(g_bf_dev, read_write_cmd, dma_size);
#else
            spi_send_cmd(g_bf_dev, g_bf_dev->image_buf, g_bf_dev->image_buf, dma_size);
#endif
            spi_set_dma_en(0);
        }
        break;
...
#ifdef NEED_OPT_POWER_ON2V8
        value = gpio_get_value(g_bf_dev->power_2v8_gpio);
        BF_LOG("gpio power status value : %d", value);
        if (copy_to_user((void __user*)arg, &value, sizeof(u32) * 1) != 0 ) {
            error = -EFAULT;
        }
#else
...
static void bf_platform_info(void)
{
    ...
    memcpy(bl_platform, "PLATFORM_SPRD", sizeof("PLATFORM_SPRD"));
#elif defined(PLATFORM_QCOM)
    memcpy(bl_platform, "PLATFORM_QCOM", sizeof("PLATFORM_SPRD"));
#elif defined(PLATFORM_RK)
    memcpy(bl_platform, "PLATFORM_RK", sizeof("PLATFORM_RK"));
#elif defined(MTK_ANDROID_L)
    memcpy(bl_platform, "MTK_ANDROID_L", sizeof("MTK_ANDROID_L"));
#elif defined(PLATFORM_MTK)
    memcpy(bl_platform, "PLATFORM_MTK", sizeof("PLATFORM_MTK"));
#elif defined(PLATFORM_MT6739)
    memcpy(bl_platform, "PLATFORM_MT6739", sizeof("PLATFORM_MT6739"));
#elif defined(PLATFORM_MT676x)
    memcpy(bl_platform, "PLATFORM_MT676x", sizeof("PLATFORM_MT676x"));
#elif defined(USE_SPI1_4GB_TEST)
    memcpy(bl_platform, "USE_SPI1_4GB_TEST", sizeof("USE_SPI1_4GB_TEST"));
#else
    memcpy(bl_platform, "undefined", sizeof("undefined"));
#endif

#if defined (TEE_TK)
    memcpy(tee_platform, "TK", sizeof("TK"));
#elif defined(TEE_BEANPOD)
    memcpy(tee_platform, "BEANPOD", sizeof("BEANPOD"));
#elif defined(BF_REE)
    memcpy(tee_platform, "REE", sizeof("REE"));
#elif defined(other)
    memcpy(tee_platform, "other", sizeof("other"));
#else
...
/* prize added by tangcong, Beitelai-bf3690 fingerprint info, 20200722-start */
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
        sprintf(current_fingerprint_info.chip,"bf3690");
        //strcpy(current_fingerprint_info.id, GF_LINUX_VERSION);
        strcpy(current_fingerprint_info.vendor,"Beitelai");
