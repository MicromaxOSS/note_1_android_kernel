...
/* debug log setting */
u8 fpsensor_debug_level = ERR_LOG;//INFO_LOG;
...
#define FPSENSOR_SPI_BUS_DYNAMIC 0
...
int fpsensor_spidev_dts_init(fpsensor_data_t *fpsensor)
{
    ...
    int ret = 0;
    FUNC_ENTRY();
    node = of_find_compatible_node(NULL, NULL, "mediatek,mt6765-fpc");//"mediatek,icnfpsensor");
    if (node) {
        ...
        fpsensor->eint_as_int = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_eint_as_int");//"finger_eint");
        ...
        fpsensor->fp_rst_low = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_pins_rst_low");//"finger_rst_low");
        ...
        fpsensor->fp_rst_high = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_pins_rst_high");//"finger_rst_high");
...
//------------------------------ree
        fpsensor->fp_spi_miso = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_mode_as_mi");//"spi_mi_low");
        ...
        fpsensor->fp_spi_mosi = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_mode_as_mo");//"spi_mo_low");
        ...
        fpsensor->fp_cs_high = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_pins_cs_high");//"spi_cs_high");
        ...
        fpsensor->fp_cs_low = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_pins_cs_low");//"spi_cs_low");
        if (IS_ERR(fpsensor->fp_cs_low)) {
		 ...
		 fpsensor->fp_cs_set = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_mode_as_cs");//"spi_cs_set");
        if (IS_ERR(fpsensor->fp_cs_set)) {
            ret = PTR_ERR(fpsensor->fp_cs_set);
            fpsensor_debug(ERR_LOG, " Cannot find fp pinctrl spi_cs_set!\n");
            return ret;
        }
        fpsensor->fp_spi_clk = pinctrl_lookup_state(fpsensor->pinctrl1, "fpc_mode_as_ck");//"spi_mclk_low");
...
static int fpsensor_probe(struct spi_device *spi)
{
    ...
    if (fpsensor_check_HWID(fpsensor_dev) == 0) {
		...
		pinctrl_select_state(g_fpsensor->pinctrl1, g_fpsensor->fp_cs_set);
        goto err2;
    }
