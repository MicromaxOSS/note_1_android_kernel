...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	static uint32_t nav_input_backup = 0;//prize add by mhy
...
/* prize added by penggy, for fingerprint resume, 20190221 begin */		
        enable_irq_wake(gf_dev->irq);
/* prize added by penggy, for fingerprint resume, 20190221 end */	
		gf_dev->irq_count = 1;
	...
	case GF_IOC_NAV_EVENT:
		...
		switch (nav_event) {
		    ...
		    nav_input_backup = GF_NAV_FINGER_DOWN;
		    gf_debug(ERR_LOG, "nav finger down");
			break;

		    case GF_NAV_FINGER_UP:
		//prize add by mhy start
		if (nav_input_backup != GF_NAV_FINGER_DOWN) {
		    input_report_key(gf_dev->input, nav_input_backup, 0);
		    input_sync(gf_dev->input);
		}
		    nav_input_backup = GF_NAV_FINGER_UP;
		...
		if ((nav_event != GF_NAV_FINGER_DOWN) && (nav_event != GF_NAV_FINGER_UP)) {
		    ...
		    input_sync(gf_dev->input);
		if (nav_input_backup == GF_NAV_FINGER_UP) {//prize add by mhy
		    input_report_key(gf_dev->input, nav_input, 0);
		    input_sync(gf_dev->input);
		}//prize add by mhy
		    nav_input_backup = nav_input;//prize add by mhy
		}
		break;
...
/* prize added by mahuiyin, goodix-gf5118m fingerprint info, 20190408-start */
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
        sprintf(current_fingerprint_info.chip,"GF5118M_tee");
