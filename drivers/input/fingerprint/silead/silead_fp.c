...
#include "silead_fp.h"
/* start add uuid_fp ..................ztg*/
#include "../../../../misc/mediatek/teei/300/common/include/tee_client_api.h"
#include "../../../../misc/mediatek/teei/300/tz_driver/include/teei_fp.h"
struct TEEC_UUID vendor_uuid = {0x511ead0a, 0x0000, 0x0000, { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 } };
/* end add uuid_fp ..................ztg*/
...
static int silfp_probe(struct spi_device *spi)
{
    ...
    // add ztg
    LOG_MSG_DEBUG(INFO_LOG, "[%s] init ztg \n",__func__);
    memcpy(&uuid_fp, &vendor_uuid, sizeof(struct TEEC_UUID));
    LOG_MSG_DEBUG(INFO_LOG, "[%s] out ztg \n",__func__);

#ifdef CONFIG_PRIZE_HARDWARE_INFO
	sprintf(current_fingerprint_info.chip, "gsl6157");
	sprintf(current_fingerprint_info.id, FP_DEV_VERSION);
	strcpy(current_fingerprint_info.vendor, "silead");
