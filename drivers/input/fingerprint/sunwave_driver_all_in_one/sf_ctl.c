...
static struct spi_board_info spi_board_devs[] __initdata = {
    [0] = {
        .modalias = "sunwave-fp",
        .bus_num = 0,
#ifdef PRIZE_SW_CHIP_SELECT
		.chip_select = PRIZE_SW_CHIP_SELECT,
#else
        .chip_select = 0,
#endif
        .mode = SPI_MODE_0,
...
static ssize_t sunwave_chip_status_show(struct device* dev,
                                      struct device_attribute* attr, char* buf)
{
    int len = 0;
    len += sprintf((char*)buf, "%d\n", sunwave_driver_status);
    return len;
}

static DEVICE_ATTR(chip_status, S_IRUGO | S_IWUSR, sunwave_chip_status_show, NULL);
...
/* prize added by mahuiyin, goodix-gf5118m fingerprint info, 20190408-start */
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
        sprintf(current_fingerprint_info.chip,"SW9022");
        //strcpy(current_fingerprint_info.id, GF_LINUX_VERSION);
        strcpy(current_fingerprint_info.vendor,"sunware");
